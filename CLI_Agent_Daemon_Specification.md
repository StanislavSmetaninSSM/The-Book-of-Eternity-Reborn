...
If the soul has `soul_state.afterlifeCombatProfile.capstones.lightIncarnate`, every contested afterlife spiritual conflict `diceAudit.modifierBreakdown.player[]` must include the `light_incarnate` modifier. Required value is `+8` when the player is lead contestant, `+4` when the player is supporter/champion-side contributor, plus `+4` extra for `force_incarnation`, `force_binding`, or `break_binding`. Do not hide this capstone bonus in narrative prose or in Mortal combat fields.

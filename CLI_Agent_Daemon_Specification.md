...
- Quest 4 of a Saref Guardian line is resolved through playable `memoryScene` layer `Воспоминание`, not a Mortal World incarnation and not `Memory Gates`. Use `sarefMainStoryUpdate.mode=record_memory_scene`; the scene must include `role`, `boundaries[]`, 3-5 `abilities[]`, `requiredStoryNodes[]`, `successCondition`, `closureTarget`, `resolvedAtTurn`, and then persist `guardianQuestlines[].questStates[].memorySceneProof` with `layer="Воспоминание"` and `successConditionSatisfied=true`. Do not grant quest-4 `sarefRevelations[]` or `sarefAdvantages[]` without that proof.

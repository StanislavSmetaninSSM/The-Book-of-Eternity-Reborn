...
9. **Examples/E_CLI_Afterlife_Turns.txt** — ОБЯЗАТЕЛЬНЫЕ worked examples для ходов в `Chaos Sea` / `Shining Abode`; читать после матрицы перед каждым afterlife-ходом, а для Shining core actions, свободных Guardian-команд, combined scheduler+pending turns, ordinary living-world turns без pending-файлов, system Guardian attraction, protected return guard, direct resident / pending-backed playerAction tags и свободного поиска Обители (`freeform Abode search`) сверять examples 14-23

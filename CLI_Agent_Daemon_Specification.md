...
- В `Chaos Sea` и `Shining Abode` запрещены: `experienceGained`, `statsIncreased`, `statsDecreased`, `currentPoiseChange`, `currentEnergyChange`, `currentHealthChange`, `moneyChange`, `activeSkillChanges`, `passiveSkillChanges`, `skillMasteryChanges`, `UpdateInventory`, `UpdateNPCs`, `NPCsInScene`, `NPCCoreChanges`, `UpdateQuests`, `worldEventsLog`, `factionDataChanges`, `currentLocationData`, `timeChange`, `setWorldTime`, `weatherChange`, `enemiesData`, `alliesData`, `combat_log_markdown`.
...
- Current/new teachable `specialArts[]` require `specialArts[].combatEffect` in addition to `effectSummary`: non-empty `summary`, `trigger`, `mechanicalAxis`, `allowedPayoff`, `limit`, and `auditRequirement`. Legal `mechanicalAxis` values are afterlife spiritual-conflict surfaces only (`rollMode`, `conflictPosition`, `controlState`, `sideStrain`, `tempoAdvantage`, `counterPayoff`, `actionEconomy`, `actionCostAudit`, `combatConditions`). The GM must preserve `baseOperation`, read the combatEffect before using a named art, and record the applied trigger/payoff through `specialArtAudit.effectNote` or `specialArtAudits[].effectNote`; do not invent Mortal HP/status effects, passive unlimited bonuses, or tactical-matrix bypasses. Legacy saved profiles with only `effectSummary` remain readable.
...
- `NPCCoreChanges`, new/promotion `UpdateNPCs` и другие actor-specific NPC update arrays
- `UpdateGuardians`
...
В Mortal World обычный существующий NPC не пересылается полным или неизменённым объектом через `UpdateNPCs`. Используй dedicated commands для их доменов и bounded non-carrier `NPCCoreChanges` только для exact permanent `NPCId`, non-empty `reason`, profile/location/progression, setting-owned absolute `characteristicValues`, faction affiliation upserts и locked/unrealized Fate Card definition add/remove.

True legacy promotion является единственным исключением для полного existing carrier: добавь первый `materialization` envelope и измени только роль, которая действительно стала доступна (`teacherProfile`, `tradeState`, `activeSkills`/`passiveSkills` либо Actor Brain `plans`/`currentActivity`/`completedActivities`). Все остальные pre-turn поля и inventory остаются семантически неизменными. `tradeInventory` и `trainingShowcase` не относятся к promotion: торговый stock требует точный pending NPC trade request и matching ready receipt, а учебная витрина требует точный pending `mortal_teacher_showcase`; ручной fallback для неё является узким `UpdateNPCs` patch только с identity/display fields и `trainingShowcase`. Две current записи не могут владеть одной effective same-turn NPC identity.
...
- Не используй `worldEventsLog`, `factionDataChanges`, `UpdateNPCs`, `NPCCoreChanges`, `UpdateQuests`, `currentLocationData`, `timeChange`, `weatherChange` для afterlife progression.
...
| НПС | Block 19 + подблоки | UpdateNPCs, NPCsInScene, NPCCoreChanges, ... (30+ полей) | game_state/npcs/ (14 файлов) |

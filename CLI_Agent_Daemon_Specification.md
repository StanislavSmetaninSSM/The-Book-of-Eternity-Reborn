...
- если один repair request перечисляет и canonical, и player-facing цели, GM сначала завершает все canonical-записи и только затем переписывает зависящие от них narrative/interface artifacts; клиент сравнивает output с последней фактической записью repaired canonical target, а не со временем создания request/dispatch/start, и при ненаблюдаемой записи блокирует принятие

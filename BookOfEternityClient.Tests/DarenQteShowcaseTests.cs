...
public sealed class DarenQteShowcaseTests : IDisposable
{
    ...
    private const string OldDarenApproachManorSuccessText =
    ...
    private const string DarenApproachManorSuccessText = """
        ...
        У самой кладки он остановил дыхание и позволил дождю считать вместо сердца. Камень под пальцами был холоден, в трещинах пахло сырой землёй и садовой гнилью, но рука Дарена не сорвалась на поспешность: пальцы нашли старый выступ, плащ лёг к боку, сапог встал на мох без хруста. За спиной страж тихо выругался на мокрый фитиль, собака у конюшни подняла морду и снова спрятала её под лапу; ни один взгляд не успел собрать его силуэт из тени и дождя.
    ...
    [Fact]
    public void DarenApproachManorSuccess_ReadsAsCleanStealthAftermathWithoutMechanicDrift()
    {
        var route = QteSceneService.GetDarenShowcaseRoute();
        var beat = Assert.Single(route.Beats, item => item.BeatId == "approach_manor");
        ...
        Assert.True(
            ...
            "Daren approach_manor success should still bridge directly into Mira's informant beat.");
        ...
        Assert.True(CountSentences(text) >= 7,
            "Daren approach_manor success should unfold across several aftermath sentences.");
        Assert.True(CountOccurrences(text, "Дарен") >= 3,
            "Daren approach_manor success should keep Daren as the active point-of-view protagonist.");

        foreach (var (context, termGroups) in new (string Context, string[][] TermGroups)[]
        {
            ("old linden manor wall setting", [["лип"], ["помест"], ["стен"], ["сад", "садов", "камн"]]),
            ("lantern blind spot and shadows", [["фонар"], ["слеп", "промежут", "тень"], ["свет", "луч"], ["скольз", "соскольз"]]),
            ("patrol guard pressure avoided", [["патрул", "страж", "караул"], ["выруг", "собак", "морду", "фитил"], ["не подня", "не увид", "не услыш", "отверн", "унёс"]]),
            ("clean no witness no alarm no evidence trail", [["свидетел"], ["тревог", "оклик", "крик"], ["ули", "след", "отпечат"], ["не остав", "не получил", "не наш", "без след", "молч"]]),
            ("Daren breath steps hands body control", [["Дарен"], ["дых", "вдох", "выдох"], ["ладон", "пальц", "рук"], ["шаг", "сапог", "ступ"], ["плеч", "колен", "тело"], ["ровн", "держ", "удерж", "медлен"]]),
            ("wet night stone garden approach", [["мокр", "дожд", "сыр", "влаж"], ["камн", "кладк", "плит"], ["сад", "трава", "листь"]]),
            ("Mira informant continuity", [["Мира"], ["Ночная Нить", "информ", "шёп", "шеп"], ["навес", "задн", "дорог"], ["дальш", "вперёд", "вперед", "пошёл"]])
        })
        {
        ...
        AssertNoPlayerFacingTechnicalTerms("approach_manor success aftermath", text);
    }

    [Fact]
    public void DarenApproachManorPartial_ReadsAsCostlyStealthAftermathWithoutMechanicDrift()
    {
        var route = QteSceneService.GetDarenShowcaseRoute();
        var beat = Assert.Single(route.Beats, item => item.BeatId == "approach_manor");
        var nextBeat = Assert.Single(route.Beats, item => item.BeatId == "informant_parley");
        ...
        Assert.True(
            FindChapterIndex(route.Offer, "approach_manor") < FindChapterIndex(route.Offer, "informant_parley"),
            "Daren approach_manor partial should still bridge directly into Mira's informant beat.");
        ...
        Assert.True(CountSentences(text) >= 7,
            "Daren approach_manor partial should unfold across several aftermath sentences.");
        Assert.True(CountOccurrences(text, "Дарен") >= 3,
            "Daren approach_manor partial should keep Daren as the active point-of-view protagonist.");

        foreach (var (context, termGroups) in new (string Context, string[][] TermGroups)[]
        {
            ("old linden manor wall setting", [["лип"], ["помест"], ["стен"], ["камн", "кладк"], ["трав", "мох", "сад"]]),
            ("lantern and shadow approach atmosphere", [["фонар"], ["тень", "слеп", "промежут"], ["свет", "луч"], ["калит"], ["стар", "лип"]]),
            ("partial cost trace and suspicion", [["ветк", "хруст", "слом"], ["след", "ули", "отпечат", "смят"], ["сомнен", "подозр", "провер", "повод"], ["задерж", "вернул", "ещё", "еще"]]),
            ("guard patrol dog or witness pressure", [["страж", "патрул", "караул"], ["пёс", "собак", "конюш"], ["оклик", "крик", "лицо", "свидетел"], ["фонар", "огонь", "луч"]]),
            ("Daren body control under pressure", [["Дарен"], ["дых", "вдох"], ["ладон", "пальц", "рук"], ["плеч", "колен", "тело", "ребр"], ["сжал", "прижал", "удерж", "застав"]]),
            ("wound delay and continued route progress", [["боль", "содрал", "поцарап", "кров"], ["медлен", "задерж", "быстр"], ["скольз", "пошёл", "прийти", "путь"], ["дальш", "открыт", "следующ"]]),
            ("Mira informant continuity", [["Мира"], ["Ночная Нить", "информ", "шёп", "шеп"], ["навес", "задн", "дорог"], ["дальш", "вперёд", "вперед", "прийти"]])
        })
        {
    ...
    [Fact]
    public void DarenInformantParleySuccess_ReadsAsCleanMiraTrustAftermathWithoutMechanicDrift()
    {
        var route = QteSceneService.GetDarenShowcaseRoute();
        var beat = Assert.Single(route.Beats, item => item.BeatId == "informant_parley");
        ...
        Assert.True(
            ...
            "Daren informant_parley success should still bridge directly into the hook-and-line beat.");
        ...
        Assert.True(text.Length >= 900,
            "Daren informant_parley success should be a substantial clean Mira trust aftermath insert, not a one-sentence result notification.");
        Assert.True(CountSentences(text) >= 8,
            "Daren informant_parley success should unfold across several aftermath sentences.");
        Assert.True(CountOccurrences(text, "Дарен") >= 4,
            "Daren informant_parley success should keep Daren as the active point-of-view protagonist.");

        foreach (var (context, termGroups) in new (string Context, string[][] TermGroups)[]
        {
            ("Mira Night Thread presence and softened suspicion", [["Мира"], ["Ночная Нить", "нить"], ["подозр", "довер", "повер", "прин", "кив"]]),
            ("correct password and guard-shift answer", [["парол"], ["смен", "караул"], ["точн", "правильн"], ["назвал", "ответ", "сказал", "произн"]]),
            ("source protection and reduced exposure risk", [["источник", "связн", "имя"], ["не выда", "не привед", "береж", "сохран", "прикры"], ["страж", "свидетел", "порог"], ["тих", "молч", "без крик", "не разбуд"]]),
            ("Lukyan and Orvald usable information", [["Лукьян"], ["галер", "ключник"], ["Орвальд"], ["погон", "сам", "вед"]]),
            ("wet awning social atmosphere", [["навес"], ["мокр", "дожд", "капл"], ["лент"], ["нож", "ладон", "запяст", "пальц"], ["шеп", "голос"]]),
            ("Daren body voice and breath control", [["Дарен"], ["дых", "вдох", "выдох"], ["голос", "сказал", "произн"], ["ладон", "плеч", "пальц", "тело"], ["ровн", "спокой", "тих"]]),
            ("hook-and-line continuity", [["крюк"], ["леск", "шнур"], ["стен", "башн", "двор"], ["дальш", "вперёд", "вперед", "ушёл", "пошёл"]])
        })
        {
            AssertContainsEveryTermGroup($"informant_parley success {context}", text, termGroups);
        }
        ...
        Assert.Contains("Складной крюк ушёл в темноту слишком резко", gadgetAction.FailText);
        AssertDownstreamDarenResultSurfacesPreserved(route.Offer);
        AssertNoPlayerFacingTechnicalTerms("informant_parley success aftermath", text);
    }

    [Fact]
    public void DarenInformantParleyPartial_ReadsAsMixedMiraTrustAftermathWithoutMechanicDrift()
    {
        var route = QteSceneService.GetDarenShowcaseRoute();
        var beat = Assert.Single(route.Beats, item => item.BeatId == "informant_parley");
        var nextBeat = Assert.Single(route.Beats, item => item.BeatId == "gadget_infiltration");
        ...
        Assert.True(
            FindChapterIndex(route.Offer, "informant_parley") < FindChapterIndex(route.Offer, "gadget_infiltration"),
            "Daren informant_parley partial should still bridge directly into the hook-and-line beat.");
        ...
        Assert.False(
            ...
            "Daren informant_parley partial should reject the old one-sentence result notification.");
        Assert.NotEqual(action.SuccessText, action.PartialText);
        Assert.NotEqual(action.SuccessText, action.FailText);
        Assert.NotEqual(action.PartialText, action.FailText);

        Assert.True(text.Length >= 800,
            "Daren informant_parley partial should be a substantial mixed Mira aftermath insert, not a one-sentence result notification.");
        Assert.True(CountSentences(text) >= 7,
            "Daren informant_parley partial should unfold across several aftermath sentences.");
        Assert.True(CountOccurrences(text, "Дарен") >= 3,
            "Daren informant_parley partial should keep Daren as the active point-of-view protagonist.");

        foreach (var (context, termGroups) in new (string Context, string[][] TermGroups)[]
        {
            ("Mira Night Thread pressure and body language", [["Мира"], ["Ночная Нить", "нить"], ["лент"], ["нож", "рукоят", "ладон", "пальц", "запяст"]]),
            ("coin debt and imperfect bargain", [["монет", "кошел"], ["долг", "плат", "цена", "торг"], ["бер", "прин", "спрят"]]),
            ("incomplete Lukyan and Orvald information", [["Лукьян", "ключник"], ["Орвальд", "капитан"], ["не наз", "остав", "не прода", "удерж", "скры", "следующ"]]),
            ("source pressure and future consequence", [["источник", "связн"], ["страж", "свидетел", "караул"], ["след", "порог", "погон", "подозр", "будущ", "завтра"]]),
            ("wet awning social atmosphere", [["навес"], ["мокр", "дожд", "капл"], ["шёп", "шеп", "голос"], ["двор", "дорог", "караул", "страж"]]),
            ("Daren breath voice and body control", [["Дарен"], ["дых", "вдох", "выдох"], ["голос", "сказал", "прошеп"], ["ладон", "плеч", "пальц", "тело"], ["ровн", "тих", "сдерж", "застав"]]),
            ("hook-and-line continuity", [["крюк"], ["леск", "шнур"], ["стен", "башн", "балкон", "двор"], ["дальш", "вперёд", "вперед", "ушёл", "пошёл", "повернул"]])
        })
        {
            AssertContainsEveryTermGroup($"informant_parley partial {context}", text, termGroups);
        }
    ...
    [Fact]
    public void DarenHideoutReturn_ReadsAsUnderBridgeHideoutPageWithoutMechanicDrift()
    {
        var route = QteSceneService.GetDarenShowcaseRoute();
        var beat = Assert.Single(route.Beats, item => item.BeatId == "hideout_return");
        var (chapter, action) = RequiredChapterAction(route.Offer, "hideout_return");
        var text = chapter.Narrative?.Trim() ?? "";
        var sentenceCount = CountSentences(text);
        var darenMentions = CountOccurrences(text, "Дарен");

        Assert.Equal("daren_qte_showcase", route.RouteId);
        Assert.Equal("Убежище под мостом", beat.Title);
        Assert.Equal(beat.PlayerText, chapter.Narrative);
        Assert.Equal("hideout_return_action", action.ActionId);
        Assert.Equal("BranchChoice", action.Check.Type);
        Assert.Equal(Characteristics.Wisdom, action.Check.PrimaryCharacteristic);
        Assert.Equal(3, action.Check.BaseDifficulty);
        var config = RequiredConfig(action);
        Assert.Equal("success", RequiredString(config, "choiceGrade"));
        Assert.Equal("Спрятать посох и зачистить след", action.Label);
        ...
        {
            Assert.True(string.IsNullOrWhiteSpace(target.NextChapterId),
                $"Daren hideout_return {grade} route should end in terminal outcome, not a next chapter.");
            Assert.Equal("daren_hideout_return", target.TerminalOutcomeId);
        }

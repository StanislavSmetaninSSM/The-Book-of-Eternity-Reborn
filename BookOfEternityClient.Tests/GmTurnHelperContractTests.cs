...
public sealed class GmTurnHelperContractTests
{
    ...
    public void DaemonContextPack_GeneratesCompactTurnAndRepairTemplates()
    {
        ...
        Assert.Contains("- Данные профиля:", daemon, StringComparison.Ordinal);
        Assert.Contains("- Мотивация:", daemon, StringComparison.Ordinal);
        Assert.Contains("- Ограничения:", daemon, StringComparison.Ordinal);
        Assert.Contains("- Варианты стратегий:", daemon, StringComparison.Ordinal);
        Assert.Contains("Выгода:", daemon, StringComparison.Ordinal);
        Assert.Contains("Риск:", daemon, StringComparison.Ordinal);
        Assert.Contains("- Выбранная стратегия:", daemon, StringComparison.Ordinal);
        Assert.Contains("- Почему альтернативы отвергнуты:", daemon, StringComparison.Ordinal);
        Assert.Contains("- Изменения состояния:", daemon, StringComparison.Ordinal);
        Assert.Contains("actor_memory_persistence_repair", daemon, StringComparison.Ordinal);
        Assert.Contains("update only the same actor's `Изменения состояния:` line", daemon, StringComparison.Ordinal);
        Assert.Contains("does not invalidate or rewrite narrative/interface output", daemon, StringComparison.Ordinal);
        Assert.Contains("canonical top-level `entries[]` root", daemon, StringComparison.Ordinal);
        Assert.Contains("schemaVersion is unsupported in guardian_thought_journal.json", daemon, StringComparison.Ordinal);
        Assert.Contains("First materialization of an NPC, resident, afterlife entity, political actor, or faction", daemon, StringComparison.Ordinal);
        Assert.Contains("exact stable actor id even when another actor shares the display name", daemon, StringComparison.Ordinal);
        Assert.Contains("shining_faction_relevant_actor_missing_strategic_memory_delta", daemon, StringComparison.Ordinal);
        Assert.Contains("afterlife_relevant_actor_missing_canonical_memory_owner", daemon, StringComparison.Ordinal);
        Assert.Contains("shiningFactionStrategicMemoryUpdates", daemon, StringComparison.Ordinal);
        Assert.Contains("shiningFactionChronicleUpdates", daemon, StringComparison.Ordinal);
        Assert.Contains("never use a shiningFactionStrategicMemoryUpdates rewrite alone", daemon, StringComparison.Ordinal);

...
- Natural 20/1 is bounded critical logic, not arbitrary fiat. Player natural 20 or opposition natural 1 means at least `player_success`; player natural 1 or opposition natural 20 means at most `opposition_success`; opposed natural criticals cancel back to the margin band. If this changes the margin-derived band, add `diceAudit.criticalResult` with `playerNaturalRoll`, `oppositionNaturalRoll`, `marginOutcomeBand`, `normalizedOutcomeBand`, `scaleLimit`, and `narrativeConstraint`; the narration must stay plausible for the current action and relative side strength.

...
- `soulDissipationTier` is informational and dangerous: if it is above zero, the profile should clearly warn that the entity can potentially kill/disperse souls after victory when its motives allow it. Whether the entity chooses to do so remains roleplay/contract context, not automatic execution.

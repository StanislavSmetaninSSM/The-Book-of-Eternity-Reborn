...
- `afterlifeEntityProfileUpdates` / `afterlifeEntityCustomStateChanges` for Профили сущностей посмертия and their customStates in `game_state/meta/afterlife_entity_profiles.json`
...
- `game_state/meta/afterlife_entity_profiles.json` ← `afterlifeEntityProfileUpdates`, `afterlifeEntityCustomStateChanges`, `afterlifeEntityProfiles` (Профили сущностей посмертия: `profiles[]` with `actorType`, `actorId`, `displayName`, realm/location, `currencies`, `progression`, `standardArts`, `specialArts`, `customStates`, `soulDissipationTier`, `progressionStrategy`, warnings, and `ledger`)

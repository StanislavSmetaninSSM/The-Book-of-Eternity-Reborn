...
- `afterlifeEntityProfileUpdates` / `afterlifeEntityCustomStateChanges` / `afterlifeEntityProgressionOverrides` for Профили сущностей посмертия, their customStates, and explicit GM progression overrides in `game_state/meta/afterlife_entity_profiles.json`
...
- `standardArts` are the normal spiritual actions (`pressure`, `guard`, `counter`, `maneuver`, `binding`, `force_binding`, `break_binding`, `incarnation_resistance`, `champion_coordination`, `recover_spiritual_power`) with tier `0..5`. `specialArts[].baseOperation` must name one of those actions; if a special art effect exists and is used in a conflict, the GM must include the special-effect note required by the spiritual conflict contract.

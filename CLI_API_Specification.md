...
- `afterlifeEntityProfileUpdates` / `afterlifeEntityCustomStateChanges` / `afterlifeEntityProgressionOverrides` / `afterlifeSpecialArtLearningReceipts` for Профили сущностей посмертия, their customStates, explicit GM progression overrides, and special-art learning in `game_state/meta/afterlife_entity_profiles.json`
...
- If the GM does not write `afterlifeEntityProgressionOverrides[]`, the client deterministically applies entity progression from existing `progression_report.json` afterlife/Shining cycle counts: income is added, one affordable `progressionStrategy.priorityOrder` upgrade is bought, `lastAutoProgressionCycleKey` is set, and `progressionLedger[]` records income/spending/upgrades. Overrides must include `cycleKey`, `reason`, `summary`, and explicit deltas.

...
- Supported operation types are `pressure`, `counter`, `guard`, `maneuver`, `break_binding`, `force_binding`, `force_incarnation`, `withdraw`, `surrender`, and `negotiate`; supported outcomes are `success`, `partial_success`, `blocked`, `countered`, `setback`, and `no_effect`.

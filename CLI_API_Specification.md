...
- Client-owned `return_to_chaos_sea` is allowed only when there are no active Shining pending contracts and no legacy `shining_abode_state.json.pendingNativeFactionDiscovery`. Resolve or repair `pending_shining_abode_actions.json`, Shining trade inventory requests, Shining founding requests, Shining realignment requests, Shining leadership transition requests, and legacy native discovery before sealing the Abode. A valid pending file whose root is exactly an empty `requests[]` array is stale client-owned clutter and may be deleted by runtime health logic; malformed files or non-empty `requests[]` block the return fail-closed until repair or explicit cleanup.

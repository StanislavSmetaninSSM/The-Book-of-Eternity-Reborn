...
- `afterlifeEntityProfileUpdates` for Профили сущностей посмертия in `game_state/meta/afterlife_entity_profiles.json`
...
- `game_state/meta/afterlife_entity_profiles.json` ← `afterlifeEntityProfileUpdates`, `afterlifeEntityProfiles` (Профили сущностей посмертия: `profiles[]` with `actorType`, `actorId`, `displayName`, realm/location, `currencies`, `progression`, `standardArts`, `specialArts`, `soulDissipationTier`, `progressionStrategy`, warnings, and `ledger`)

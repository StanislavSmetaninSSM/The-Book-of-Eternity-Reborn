...
- `game_state/meta/main_story_saref_state.json` ← `sarefMainStoryState` or `sarefMainStoryUpdate` for hidden `Крылья над Бездной` state. The `/сареф найти_крылья` pending contract closes only through `sarefMainStoryUpdate.mode=reveal_wings|refuse_wings|block_wings`, matching `requestId`, positive `resolvedAtTurn`, and `wingsInfiltration.status=revealed|refused|blocked`.

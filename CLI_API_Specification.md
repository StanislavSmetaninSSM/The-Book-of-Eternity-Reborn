...
- Client-owned `return_to_chaos_sea` is allowed only when there are no active or malformed Shining pending contracts and no legacy `shining_abode_state.json.pendingNativeFactionDiscovery`. A valid explicit-empty `{ "requests": [] }` Shining pending file is a stale client artifact and may be deleted by runtime; a malformed file or any non-empty `requests[]` blocks sealing until repair/closure. Resolve or repair `pending_shining_abode_actions.json`, Shining trade inventory requests, Shining founding requests, Shining realignment requests, Shining leadership transition requests, and legacy native discovery before sealing the Abode.

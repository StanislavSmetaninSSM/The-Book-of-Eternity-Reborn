...
**Constraints**: Preserve route mechanics, action ids, check type/config, routing, grade identities, score deltas, rewards, endpoints, runtime state, browser/console parity, #997 success result text, #999 fail result text, downstream keykeeper/cabinet/rune result text, and frontend files.

**Scale/Scope**: One result surface, one focused test guard, one Spec Kit feature directory.

**Source Issue(s)**: [#998](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/998), parent [#955](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/955), source scene [#972](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/972), completed sibling [#997](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/997), future sibling [#999](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/999).
...
- **GitHub traceability**: PASS. Spec, plan, tasks, checklist, and contract reference #998 and parent #955.
...
```text
specs/998-daren-gallery-partial/
├── spec.md
├── plan.md
├── contracts/
...
**Structure Decision**: Keep route result prose in the existing shared C# route data and extend the existing Daren showcase test suite. Do not add frontend, endpoint, state, reward, or documentation contract files outside the #998 Spec Kit artifacts unless verification exposes a real issue.

## TDD Strategy

1. Add a focused test such as `DarenStealthCrossingPartial_ReadsAsCostlyGalleryAftermathWithoutMechanicDrift` in `BookOfEternityClient.Tests/DarenQteShowcaseTests.cs` before changing the partial prose.
2. The guard asserts:
   ...
   - grouped motif coverage for listening gallery surfaces, floorboard/noise trouble, Daren breath/body/boot control, partial cost/suspicion/evidence, achieved passage despite price, and next keykeeper/service-door continuity;
   - absence of default player-facing technical terms including `QTE` and score/debug framing;
   - unchanged #997 success and #999 fail surfaces plus downstream keykeeper/cabinet/rune-memory strings unless tests prove a minimal connective need.
3. Run the focused Daren test filter and record RED evidence against the existing one-sentence partial result.
4. Replace only the `stealth_crossing` action partial text in `QteSceneService.Daren.cs`.
5. Run focused and affected verification to GREEN.

## Baseline Evidence Before Implementation

- Branch `work/998-daren-gallery-partial` started from `origin/main` at `273c41c`; source issue #998 and parent #955 are the tracked tasks.
- Focused Daren baseline command: `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj -p:IsTestProject=true --filter "FullyQualifiedName~DarenQteShowcaseTests" --logger "console;verbosity=minimal"` passed: 67 passed / 0 failed / 0 skipped / 67 total.
- Affected slice baseline command: `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj -p:IsTestProject=true --filter "DarenQteShowcaseTests|QteSceneServiceTests|ValidationServiceQteTests|PromptDocumentationCoverageTests|ExampleDocumentationValidationTests|BrowserApiContractTests|BrowserFrontendWorkspaceTests" --logger "console;verbosity=minimal"` passed: 336 passed / 0 failed / 0 skipped / 336 total.
- Spec Kit prerequisite check resolved `FEATURE_DIR` to `E:\\Games\\worktrees\\boe-998-daren-gallery-partial\\specs\\998-daren-gallery-partial` with `contracts/` and `tasks.md` before Codex implementation.

## Implementation Evidence To Record During This Closure Unit

- RED focused Daren guard count and expected failure against the old `stealth_crossing_action` partial string.
...
- `stealth_crossing_action` partial literary quality against issue #998 and parent #955;
- Daren remains the active protagonist in a costly but still-progressing silent-gallery aftermath;
- #997 success, #999 fail, downstream keykeeper/cabinet/rune results #1000-#1008, route mechanics, and runtime/browser contracts remain unchanged;

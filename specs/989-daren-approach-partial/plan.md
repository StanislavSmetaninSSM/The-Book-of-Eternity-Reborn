# Implementation Plan: Daren Approach Manor Partial Literary Aftermath

**Issue**: [#989](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/989)
**Branch**: `work/989-daren-approach-partial`
**Feature directory**: `specs/989-daren-approach-partial/`

## Summary

Rewrite exactly one shared Daren QTE result surface: `approach_manor_action` partial text in `BookOfEternityClient/Services/QteSceneService.Daren.cs`. The outcome must become a substantial Russian mixed stealth aftermath for the approach to the manor, while preserving all route mechanics, same-scene sibling results, and downstream result surfaces.

## Technical Approach

- Add a focused test to `BookOfEternityClient.Tests/DarenQteShowcaseTests.cs` before production prose changes.
- The test should fail against the current terse partial text, then pass after rewriting the partial prose.
- Keep the production change inside the existing Daren route data string for `approach_manor_action` partial.
...
- Modify: `BookOfEternityClient.Tests/DarenQteShowcaseTests.cs`
  - Add `DarenApproachManorPartial_ReadsAsCostlyStealthAftermathWithoutMechanicDrift` or equivalent focused guard.
...
- Modify: `BookOfEternityClient/Services/QteSceneService.Daren.cs`
  - Replace only the second result string for `approach_manor_action` (`partial`).
- Modify: `specs/989-daren-approach-partial/tasks.md`
  - Record RED/GREEN counts, build counts, static scan, and implementation commit evidence.
- Modify: `specs/989-daren-approach-partial/checklists/requirements.md`
...
- Preserve #988 success and #990 fail text exactly unless a test proves a real accidental drift that must be reverted.

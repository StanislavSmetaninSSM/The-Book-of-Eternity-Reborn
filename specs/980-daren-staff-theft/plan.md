# Implementation Plan: Daren Scene 12 Full Literary Page

**Branch**: `work/980-daren-staff-theft` | **Date**: 2026-06-15 | **Spec**: `specs/980-daren-staff-theft/spec.md`

**Input**: Feature specification from `specs/980-daren-staff-theft/spec.md`; source GitHub issue [#980](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/980); parent [#955](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/955).

## Summary

Rewrite only Daren QTE scene `staff_theft` / "Кража посоха" into a substantial Russian dark-fantasy theft page while preserving the existing shared route action contract. Add a focused failing test first so the old synopsis is rejected and the final page proves console/browser parity through shared route data.
...
**Source Issue(s)**: [#980](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/980), parent [#955](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/955).
...
- **GitHub traceability**: PASS. Spec, plan, tasks, checklist, and contract reference #980 and parent #955.
...
**Structure Decision**: Keep route prose in the existing shared C# route data and extend the existing Daren showcase test suite. Do not add frontend, endpoint, state, reward, or documentation contract files outside the #980 Spec Kit artifacts unless verification exposes a real issue.

## TDD Strategy

1. Add `DarenStaffTheft_ReadsAsRelicTheftPageWithoutMechanicDrift` in `BookOfEternityClient.Tests/DarenQteShowcaseTests.cs` before changing the scene prose.
2. The guard asserts:
   - title and shared route data parity;
   - action id, `BalanceMeter`, characteristic, difficulty, label, routing, and score deltas;
   - substantial page length, sentence count, and Daren active POV;
   - grouped motif coverage for staff/relic and holders, rings/suspension/chime noise, Daren hands/breath/body balance, belt/strap/futlyar securing, evidence/old-lock/scratch stakes, alarm/listening-house/pursuit pressure, and BalanceMeter theft-control lead-in;
...
4. Replace only the `staff_theft` narrative / `DarenShowcaseBeat.PlayerText` in `QteSceneService.Daren.cs`.
5. Run focused and affected verification to GREEN.

## Baseline Evidence Before Implementation

- `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .specify/scripts/powershell/check-prerequisites.ps1 -Json -RequireTasks -IncludeTasks` resolved `FEATURE_DIR=E:\\Games\\worktrees\\boe-980-daren-staff-theft\\specs\\980-daren-staff-theft` with `contracts/` and `tasks.md`.
- `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj -p:IsTestProject=true --filter "FullyQualifiedName~DarenQteShowcaseTests" --logger "console;verbosity=minimal"` passed: 79 passed / 0 failed / 0 skipped / 79 total.
- `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj -p:IsTestProject=true --filter "DarenQteShowcaseTests|QteSceneServiceTests|ValidationServiceQteTests|PromptDocumentationCoverageTests|ExampleDocumentationValidationTests|BrowserApiContractTests|BrowserFrontendWorkspaceTests" --logger "console;verbosity=minimal"` passed: 348 passed / 0 failed / 0 skipped / 348 total.

## Review Requirements

Independent review must check:

- `staff_theft` literary quality against issue #980 and parent #955;
- Daren remains the active protagonist in a relic-theft/balance-control scene;
- the prose carries forward old-lock/scratch, staff-case/relic, alarm/listening-house, and pursuit pressure without adding a new social dialogue system;
- the scene naturally narrows into the existing "Удержать посох на ремне" action;
...
- Spec Kit artifacts do not stale-reference #979/orangery as the current source issue.

...
- Final sweep branch: `E:/Games/worktrees/boe-1174-console-final-sweep`, branch `work/1174-console-final-sweep`, created from `origin/main` at `bbb2c2d`.
- Final sweep baseline: Spec Kit prerequisite check for `specs/1174-console-polish-9` passed; `dotnet build BookOfEternityClient\BookOfEternityClient.csproj --no-restore` passed with 0 warnings and 0 errors.
- Final sweep focused pre-live verification: `dotnet test BookOfEternityClient.Tests\BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "ConsoleCommandOutputQualityClassifierTests|StructuredBonusDisplayTests|MortalCommandDisplaySaveTests|ChaosSeaCommandDisplaySaveTests|ShiningAbodeCommandDisplaySaveTests|SarefCommandDisplayQualityTests|AgentConsoleRecordingExplorerConsoleTests|AgentConsoleLiveInputSourceTests|AgentConsoleObservationTests"` passed 335, failed 0.
- Final live run: `C:\Temp\boe-live-e2e-1174-final-20260621-060857`; Agent Console `http://127.0.0.1:52884`; QTE disabled; GM bridge command `codex --dangerously-bypass-approvals-and-sandbox`; client and daemon PIDs from run metadata were stopped after artifact capture.
- Final live command sweep: 30 Mortal World commands returned without hangs or issue markers; artifact `mortal-command-final-sweep-summary-real.json` in the run root.
- Final live adventure: two Codex-GM Mortal World turns completed. Turn 2 required validator repair, then returned to player input. `/инв`, `/кодекс`, `/нпс`, and `/где_я` exposed the relevant post-turn clues; journal-only NPC fallback lacked drilldown before the final fix.
- Final RED NPC/system-guardian polish: `dotnet test BookOfEternityClient.Tests\BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "ExecuteAsync_NpcWithRepositoryJournalFixtureAndNoCore_ShowsKnownJournalNotes|ExecuteAsync_NpcJournalFallbackDetail_ShowsFullJournalEntriesAndBackAction|ExecuteAsync_SystemGuardiansWithEmptyLibrary_HidesLocalDirectoryPath"` failed 3, passed 0 before fallback NPC journal actions/detail and empty system-guardian message fixes.
- Final GREEN NPC/system-guardian polish: same filter passed 3, failed 0 after adding journal-only NPC detail actions and hiding empty system-guardian local path/table output.
- Final focused console-quality verification: `dotnet test BookOfEternityClient.Tests\BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "ConsoleCommandOutputQualityClassifierTests|StructuredBonusDisplayTests|MortalCommandDisplaySaveTests|ChaosSeaCommandDisplaySaveTests|ShiningAbodeCommandDisplaySaveTests|SarefCommandDisplayQualityTests|AgentConsoleRecordingExplorerConsoleTests|AgentConsoleLiveInputSourceTests|AgentConsoleObservationTests|ExplorerWebCommandServiceTests"` passed 710, failed 0.
- Final client build: `dotnet build BookOfEternityClient\BookOfEternityClient.csproj --no-restore` passed with 0 warnings and 0 errors.
- Final broad non-browser C# verification: `dotnet test BookOfEternityClient.Tests\BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName!~LocalWebUiBuiltFrontendSmokeTests" --logger "console;verbosity=minimal"` passed 4703, failed 0.
- Final whitespace check: `git diff --check` exited 0 with CRLF normalization warnings only.
- Acceptance audit updated: `docs/audits/console-acceptance-score-1180.md`, score 9/10 for non-browser, non-QTE console scope.

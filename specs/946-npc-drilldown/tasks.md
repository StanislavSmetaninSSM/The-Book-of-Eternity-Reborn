...
- [X] T014 Independent review: verify #946 acceptance, console/browser parity, no raw/debug default output, no mutation authority leak, and no scope creep into #947/#948/#949. Evidence: final review `E:/Games/codex-runs/20260611-145551-boe-946-npc-drilldown-finalreview/final.md` returned `Verdict: APPROVED`, blocking findings none; reviewer inspected `origin/main...HEAD`, ran `git diff --check`, and reran the focused 5-test #946 filter successfully.
...
- Review: final independent review `E:/Games/codex-runs/20260611-145551-boe-946-npc-drilldown-finalreview/final.md` returned `Verdict: APPROVED`, blocking findings none.
- Review-fix follow-up for independent findings: added RED coverage for default NPC drill-down leakage/mask boundaries, mechanics label wording, and section-menu back wording. RED command `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj -p:IsTestProject=true --filter "FullyQualifiedName~ExecuteAsync_NpcRichDetails_DefaultProjectionHidesInternalFieldsAndMasks|FullyQualifiedName~ExecuteAsync_NpcMechanicsSection_LabelIncludesInventoryWhenInventoryRowsExist|FullyQualifiedName~TryProcessCommand_Npcs_RichNpcShowsDetailSectionMenu" --logger "console;verbosity=minimal"` failed 3/3 before production changes: `image_prompt` leaked from default projection text, mechanics label lacked `инвентарь`, and the section-menu back choice still said `← К списку НПС`.
- Review-fix GREEN: the same focused command passed 3/3 after changing `NpcDetailSectionProjection` to allow-listed player-facing fields, removing default mask rows, and updating the two labels. Affected slice `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj -p:IsTestProject=true --filter "FullyQualifiedName~Npc|FullyQualifiedName~ExplorerWebCommandServiceTests|FullyQualifiedName~ExplorerModeSourceGuardTests|FullyQualifiedName~GameInterfaceTests" --logger "console;verbosity=minimal"` passed 298/298. Builds `dotnet build BookOfEternityClient/BookOfEternityClient.csproj --no-restore` and `dotnet build BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true` both completed with 0 warnings and 0 errors.
- Review-fix hygiene: Spec Kit prerequisite check returned `FEATURE_DIR=E:\\Games\\worktrees\\boe-946-npc-drilldown\\specs\\946-npc-drilldown` and `AVAILABLE_DOCS=["contracts/","tasks.md"]`; `git diff --check origin/main...HEAD` passed; added-line security scan outside `specs/**`/`TestResults/**` returned `NO_MATCHES`. No React/frontend files changed, so frontend verification was not run. Final independent review accepted the corrected branch with blocking findings none.

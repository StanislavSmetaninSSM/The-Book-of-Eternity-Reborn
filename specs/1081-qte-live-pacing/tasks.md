...
- [x] **T003 Confirm live evidence strategy**
  ...
  Evidence: chose deterministic in-process harness evidence. Added `BookOfEternityClient.Tests/QteLivePlayabilityTests.cs` to exercise the same live requirement/frame builders used by the console loops, and extended `QteSceneRenderingSourceGuardTests` to prove the PatternMemory reveal/input transition stays in one live display. No manual human/ConPTY visual artifact was captured in this autonomous run; the substitute evidence is deterministic test output from the live-path seams.
...
- [x] **T004 Add RED coverage for TimingBar pacing**
  ...
  RED Evidence: `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "QteLive|QteSceneRenderingSourceGuardTests" --logger "console;verbosity=minimal"` failed before production edits with 8 failed / 10 passed / 0 skipped / 18 total. TimingBar failures: `QteLive_TimingBarHardDifficultyUsesBoundedFastPacing` missing `ComputeTimingBarLiveRequirement`; `TimingBarSpeed_MustGetFasterWhenDifficultyIncreases` missing bounded requirement/timeout use.
...
- [x] **T005 Add RED coverage for PromptChain immediate-fail startup**
  ...
  RED Evidence: same RED command failed before production edits. PromptChain failures: `QteLive_PromptChainHardDifficultyKeepsReadableFirstPrompt` missing `ComputePromptChainLiveRequirement`; `PromptChainStartup_MustUseReadableFirstPromptWindow` missing explicit first-prompt grace.
...
- [x] **T006 Add RED coverage for BalanceMeter control readability**
  ...
  RED Evidence: same RED command failed before production edits. BalanceMeter failures: `QteLive_BalanceMeterFrameExplainsControlsStepAndSafeRange` missing `BuildBalanceMeterLiveFrame`; `BalanceMeterProgress_MustExplainPositionAndControlStep` did not find plain "влево на", "вправо на", or "Шаг управления" copy.
...
- [x] **T007 Add RED coverage for PatternMemory input reveal leakage**
  ...
  RED Evidence: same RED command failed before production edits. PatternMemory failures: `QteLive_PatternMemoryInputFrameDoesNotContainRevealSequence` missing deterministic reveal/input frame seam; `PatternMemory_MustReplaceRevealWithInputInSameLiveDisplay` found 2 `RunMiniGameLiveAsync` calls in `RunPatternMemoryAsync`.
...
- [x] **T008 Fix TimingBar difficulty-sensitive pacing**
  ...
  Evidence: added `TimingBarLiveRequirement`/`ComputeTimingBarLiveRequirement`, changed `RunTimingBarAsync` to use the computed requirement, faster high-difficulty tick/window, visible remaining time, and bounded timeout. Focused GREEN below shows hard difficulty has a bounded 80-220 ms success window and 1200-3000 ms timeout in the deterministic harness.
...
- [x] **T009 Fix PromptChain readable startup/input timing**
  ...
  Evidence: added `PromptChainLiveRequirement`/`ComputePromptChainLiveRequirement`; high difficulty keeps per-prompt timeout in 700-1200 ms range and first prompt in 1000-1800 ms range through explicit `FirstPromptGraceMs`.
...
- [x] **T010 Fix BalanceMeter live control copy/state display**
  ...
  Evidence: `RunBalanceMeterAsync` now uses a single `BalanceMeterMovementStep` constant for A/left and D/right movement, instructions are Russian/in-world, and `BuildBalanceMeterLiveFrame` renders position, safe range, `A/Ф или ←: влево на 10`, `D/В или →: вправо на 10`, and `Шаг управления: 10`.
...
- [x] **T012 Produce live/harness evidence for TimingBar**
  ...
  Evidence: deterministic harness test `QteLive_TimingBarHardDifficultyUsesBoundedFastPacing` in `BookOfEternityClient.Tests/QteLivePlayabilityTests.cs` passed after the fix and exercises the live requirement used by `RunTimingBarAsync`.
...
- [x] **T013 Produce live/harness evidence for PromptChain**
  ...
  Evidence: deterministic harness test `QteLive_PromptChainHardDifficultyKeepsReadableFirstPrompt` in `BookOfEternityClient.Tests/QteLivePlayabilityTests.cs` passed after the fix and exercises the live requirement used by `RunPromptChainAsync`.
...
- [x] **T014 Produce live/harness evidence for BalanceMeter**
  ...
  Evidence: deterministic harness test `QteLive_BalanceMeterFrameExplainsControlsStepAndSafeRange` in `BookOfEternityClient.Tests/QteLivePlayabilityTests.cs` passed after the fix and asserts the live frame text for controls, step, current position, and safe range.
...
- [x] **T015 Produce live/harness evidence for PatternMemory**
  ...
  Evidence: deterministic harness test `QteLive_PatternMemoryInputFrameDoesNotContainRevealSequence` in `BookOfEternityClient.Tests/QteLivePlayabilityTests.cs` passed after the fix, and `PatternMemory_MustReplaceRevealWithInputInSameLiveDisplay` in `QteSceneRenderingSourceGuardTests` passed with exactly one live display call.
...
- [x] **T016 Verify focused #1081 regression tests GREEN**
  ...
  Evidence: `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "QteLive|QteSceneServiceTests|QteSceneRenderingSourceGuardTests" --logger "console;verbosity=minimal"` passed with 0 failed / 97 passed / 0 skipped / 97 total. Earlier focused GREEN for just `QteLive|QteSceneRenderingSourceGuardTests` passed with 0 failed / 18 passed / 0 skipped / 18 total.
...
- [x] **T017 Verify broader QTE neighborhood GREEN**
  ...
  Evidence: command above passed with 0 failed / 243 passed / 0 skipped / 243 total. Reopened-issue baseline `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "Qte|Daren|RhythmPulse|ConsoleE2ESmokeTests" --logger "console;verbosity=minimal"` passed with 0 failed / 362 passed / 0 skipped / 362 total.
...
- [x] **T018 Run build, Spec Kit prerequisite check, diff hygiene, and static scan**
  ...
  Evidence:
  - `$env:SPECIFY_FEATURE_DIRECTORY='specs/1081-qte-live-pacing'; powershell.exe -NoProfile -ExecutionPolicy Bypass -File ./.specify/scripts/powershell/check-prerequisites.ps1 -Json -RequireTasks -IncludeTasks` returned `FEATURE_DIR=E:\Games\worktrees\boe-1081-qte-live-pacing\specs\1081-qte-live-pacing`, `AVAILABLE_DOCS=["contracts/","tasks.md"]`.
  - `dotnet build BookOfEternityClient/BookOfEternityClient.csproj --no-restore` succeeded with 0 warnings / 0 errors.
  - `git diff --check origin/main...HEAD` passed with no output after the local implementation commit.
  - Added-line static scan over `origin/main...HEAD` reported: `Static scan clean: 0 findings over added lines (secrets/artifacts all files; injection patterns code files only).`

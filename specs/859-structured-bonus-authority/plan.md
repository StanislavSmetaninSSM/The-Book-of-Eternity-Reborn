...
Accepted authority sources should be explicit and canonical: `structuredBonuses`, `combatEffect`, canonical consumable effect data already used by the project, or an explicit unresolved/narrative classification field documented for GM output. Structured authority must match the displayed summary through clear target/value metadata; exact display/description/summary/name text is only a mirror and does not authorize mechanics by itself. Empty objects, display-only entries, unrelated entries, or entries with contradictory metadata do not authorize a summary. Player-facing inventory rendering can continue showing resolved summaries as ordinary bonuses/effects, but unresolved or narrative-only summaries must be visibly marked so they are not presented as applied mechanics.

...
- **FR-002**: Mechanical-looking summary text MUST resolve to matching canonical authority for that displayed summary through clear structured target/value/valueType metadata in `structuredBonuses`, `combatEffect`, canonical consumable effect data, or an existing project-approved structured effect field. Exact display/description/summary/name text may mirror the visible summary but MUST NOT authorize mechanics on its own, matching display text with contradictory target/value/valueType metadata MUST be rejected, missing or unsupported structured `valueType` MUST NOT authorize applied mechanics, flat-vs-percentage value type MUST match the displayed summary, secondary combat fields such as `duration`, `poiseDamage`, and `damageThreshold` MUST NOT substitute for the primary displayed bonus value, and nested `combatEffect` target/value metadata MUST come from the same structured effect object rather than combining sibling effects. A non-empty but unrelated/empty authority array is not sufficient.

...
- [X] Add review regression coverage proving unrelated `structuredBonuses`, display-only authority, contradictory metadata, empty `customProperties`, near-numeric mismatches such as `Сила +1` vs `Сила +10`, flat-vs-percentage mismatches such as `Сила +1` vs `valueType=Percentage`, missing/unsupported structured `valueType`, split `combatEffect.effects[]` target/value metadata, and secondary combat fields such as `duration` do not authorize a mechanical-looking `bonuses` summary; verify RED.
...
- TDD RED:
  ...
  - Blocking review RED for display-only and contradictory exact-match loopholes with the same `MechanicalBonusAuthorityValidationTests` command: failed 2, passed 11, skipped 0, total 13. Failures were missing `inventory_mechanical_summary_missing_structured_authority` when `bonuses=["Сила +1"]` had only `structuredBonuses.description="Сила +1"` or had matching `description` with contradictory `target=dexterity` and `value=2`.
  ...
  - Blocking review RED for docs with the same `PromptDocumentationCoverageTests.InventoryMechanicalBonusAuthorityContract_IsDocumentedForGm` command: failed 1, passed 0, skipped 0, total 1 because `description/display text alone does not authorize mechanics` was not documented.
  - Final independent review RED for split `combatEffect.effects[]` authority with `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~MechanicalBonusAuthorityValidationTests.ValidateGameStateAsync_MechanicalBonusSummaryWithCombatEffectSplitTargetAndValue_ReportsIssue" --logger "console;verbosity=minimal"`: failed 1, passed 0, skipped 0, total 1 because `Сила +1` was authorized by `targetType=strength/value=2%` in one effect plus `targetType=dexterity/value=1%` in a sibling effect.
  - Final independent review RED for flat-vs-percent authority with `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~MechanicalBonusAuthorityValidationTests.ValidateGameStateAsync_MechanicalBonusSummaryWithPercentStructuredValue_ReportsIssue" --logger "console;verbosity=minimal"`: failed 1, passed 0, skipped 0, total 1 because `bonuses=["Сила +1"]` was authorized by `structuredBonuses.valueType="Percentage"` and `value=1`.
  - Final independent review RED for secondary combat duration values with `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~MechanicalBonusAuthorityValidationTests.ValidateGameStateAsync_MechanicalBonusSummaryWithCombatEffectDurationValue_ReportsIssue" --logger "console;verbosity=minimal"`: failed 1, passed 0, skipped 0, total 1 because `bonuses=["Сила +5"]` was authorized by a combat effect whose primary value was `2%` but `duration=5`.
  - Final independent review RED for missing/unsupported structured value types with `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~MechanicalBonusAuthorityValidationTests.ValidateGameStateAsync_MechanicalBonusSummaryWithMissingStructuredValueType_ReportsIssue|FullyQualifiedName~MechanicalBonusAuthorityValidationTests.ValidateGameStateAsync_MechanicalBonusSummaryWithUnsupportedStructuredValueType_ReportsIssue|FullyQualifiedName~MechanicalBonusAuthorityValidationTests.ValidateGameStateAsync_MechanicalBonusSummaryWithMissingCustomPropertyValueType_ReportsIssue" --logger "console;verbosity=minimal"`: failed before the validator fix because `bonuses=["Сила +1"]` could be authorized by `structuredBonuses` with missing `valueType`, unsupported `valueType="NotFlat"`, or `customProperties` with `targetStateName/changeValue` but no value type.
  - Final independent review RED for utility `String`/`Boolean` value types with `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~MechanicalBonusAuthorityValidationTests.ValidateGameStateAsync_UtilityBonusSummaryWithBooleanStructuredAuthority_DoesNotReportIssue|FullyQualifiedName~MechanicalBonusAuthorityValidationTests.ValidateGameStateAsync_StateSummaryWithStringStructuredAuthority_DoesNotReportIssue" --logger "console;verbosity=minimal"`: failed 2, passed 0, skipped 0, total 2 because documented non-numeric `valueType` authorities were not counted as meaningful structured authority for non-numeric mechanical-looking summaries.
- Focused GREEN:
  - `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~MechanicalBonusAuthorityValidationTests" --logger "console;verbosity=minimal"`: passed 21/21, 0 failed, 0 skipped.
...
- Final verification:
  - `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~Mechanical|FullyQualifiedName~Structured|FullyQualifiedName~Inventory" --logger "console;verbosity=minimal"`: passed 100/100, 0 failed, 0 skipped.
  - `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~Validation" --logger "console;verbosity=minimal"`: passed 954/954, 0 failed, 0 skipped.
  ...
  - All `Examples/*.txt` structured bonus valueType scan: no `structuredBonuses` object with `bonusType`+`value` is missing `valueType`; updated `example_validation_manifest.json` for the shifted legacy slot-fragment exemption and fixed stale examples in `E_Block_19.txt`/`E_Block_21.txt`.

...
- TDD RED:
  ...
  - Final independent review RED for missing/unsupported structured value types with `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~MechanicalBonusAuthorityValidationTests" --logger "console;verbosity=minimal"`: failed 3, passed 21, skipped 0, total 24 before the validator fix because `bonuses=["Сила +1%"]` could be authorized by `structuredBonuses`/`customProperties` with `targetType/value` but no `valueType`, and `bonusType="Сила"` alone could be treated as a target instead of a category.
...
- Focused GREEN:
  - `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~MechanicalBonusAuthorityValidationTests" --logger "console;verbosity=minimal"`: passed 24/24, 0 failed, 0 skipped.
...
- Final verification:
  - `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~Mechanical|FullyQualifiedName~Structured|FullyQualifiedName~Inventory" --logger "console;verbosity=minimal"`: passed 103/103, 0 failed, 0 skipped.
  - `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj --no-restore -p:IsTestProject=true --filter "FullyQualifiedName~Validation" --logger "console;verbosity=minimal"`: passed 957/957, 0 failed, 0 skipped.

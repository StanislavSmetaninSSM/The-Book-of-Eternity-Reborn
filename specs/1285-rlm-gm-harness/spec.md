...
- **FR-024**: Accepted turns in which a relevant Mortal or afterlife actor materially speaks, reacts, chooses, trains, trades, fights, negotiates, or closes an actor-owned contract MUST include a full Actor Brain strategy decision and a matching canonical actor-memory delta. The harness MUST reject reasoning that omits distinct alternatives, non-empty per-alternative benefit/risk, the chosen strategy, rejected-alternative rationale, or exact state changes, and MUST reject a significant actor reaction that remains only in transient prose without the actor's canonical thought/journal/ledger surface. Existing actors and factions MUST append durable history; changing mutable `gmThoughtsSummary` / `strategicMemory` fields alone is not persistence. Append-only comparison MUST use the validated snapshot entry for the exact actor-memory file and fail closed when a pre-existing surface is missing or unreadable, while authoritative pre-turn absence permits first materialization. First materialization is still a structured actor touch that MUST enter scope. When the request carries a stable actor id, append-only validation MUST bind to that exact actor even if display names collide. A memory-only repair MUST update the actor journal plus the matching Actor Brain state-change line in one narrow packet and MUST NOT invalidate unchanged player-facing narrative/interface output. Direct Guardian thought-journal repair MUST use canonical top-level `entries[]`; the first-pass `guardianThoughtJournalUpdates` command and unsupported `schemaVersion` field MUST NOT replace that on-disk root. Trivial explicitly out-of-scope scheduler contours MUST remain valid without invented actor thoughts.
- **FR-025**: An accepted-turn output-artifact repair MUST target only output files that have current validation errors. A player-facing narrative/interface refresh MUST preserve an already valid `output/debug_logs.json` unchanged. If the debug log itself requires repair, the packet and compact template MUST preserve or reconstruct the full Actor Brain decision audit, including the exact canonical journal/ledger surface in `Изменения состояния`, for Mortal NPCs, Guardians, residents, afterlife entity profiles, and Shining factions. A genuinely actorless scope MUST remain actorless and MUST NOT be forced to invent an actor.
...
- **SC-007**: A live stale-player-facing-output repair after canonical actor-state repair completes with one output refresh and does not create a subsequent Actor Brain journal-surface repair.

...
- [X] T024 [US5] Run a short live GM bridge test with an explicitly pinned Codex model and `high` reasoning effort.
...
- [X] T074 [Follow-up #1249/#1285] Pin main GM and hidden worker Codex launch commands to an explicit model and `high` reasoning, update GM-facing examples, and verify with a live Chaos Sea turn.
...
- [X] T197 [Harness/RLM follow-up #1485/#1285] Migrate main GM and hidden worker defaults to `gpt-5.6-terra high`, migrate only exact retired built-in commands, remove the retired model id from the repository, and verify both `codex exec` and the interactive ConPTY bridge startup.
- [X] T198 [Harness/RLM live-test blocker #1486/#1452/#1285] Enforce full Actor Brain strategy choice and matching persistent thought/journal deltas for materially reacting Mortal and afterlife actors, add narrow repair packets, synchronize GM guidance/examples, and repeat the captured Guardian dialogue live test on `gpt-5.6-terra high`.
- [X] T200 [Harness/RLM live-test blocker #1490/#1486/#1452/#1285] Keep accepted-turn output-only repairs diagnostic-scoped so stale narrative/interface refreshes preserve valid Actor Brain output, provide the full Actor Brain and exact memory-surface contract when `output/debug_logs.json` itself is invalid, synchronize daemon/docs/examples/source guards, and repeat the captured Guardian startup repair on `gpt-5.6-terra high`.
- [X] T201 [Harness/RLM live-test blocker #1448/#1483/#1285] Treat changing interactive GM bridge `diagnostics.outputVersion` as progress in the output-without-terminal watchdog, so an active first-Mortal bootstrap is not killed before the normal turn timeout while genuinely silent payload stalls remain bounded.
- [ ] T202 [Harness/RLM live-test blockers #1446/#1461/#1349/#1285] Pre-materialize and preserve explicit Mortal starter competencies, a concrete starter NPC/teacher anchor for apprenticeship cues, and the opening world event, with scaffold-driven validation/repair so `/навыки`, `/нпс`, and `/новости_мира` cannot be empty after a richly authored incarnation request.
- [ ] T203 [Harness/RLM live-test durability #1452/#1285] Keep autonomous Agent Console live-run sandboxes under a repository-drive `boe-live-runs` sibling by default instead of OS-cleanable `%TEMP%` on `C:`, while retaining explicit `-RunRoot` override support.

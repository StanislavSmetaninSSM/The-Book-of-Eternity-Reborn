...
6. Client deducts money and current-level XP progress, locally adds a normal new skill at mastery level 1 or updates pure mastery practice below a threshold, and writes `TrainingPurchaseReceipt`.
7. If a paid lesson crosses a mastery threshold or needs new/changed mechanical effects, the client writes `mortal_training_skill_evolution` and immediately dispatches that dedicated GM action. This must not wait for the player to close the command screen, press a key, or type the next ordinary turn input.

...
- [ ] T192 Add deterministic RED regressions for stale QTE requests against a new replacement-session offer/attempt, prompt replacement during result construction plus stale GET/invalid submit/owner mismatch, and same-owner local-UI lock replacement/release races.
- [ ] T193 Publish and require generation/interaction-revision QTE tokens, bind prompt construction before form generation and invalidate stale snapshots first, and give every local-UI acquisition one exact generation/owner/lease token while keeping replacement release inside the old generation.
- [ ] T194 Add deterministic RED regressions for crafted lock descendants and exact-node directory/reparse shapes across save/load/New Game; Guardian nonzero-turn relative reads; gacha and realm hard-link/swap-back pending-turn reads; and link-count changes after initial runtime/save validation.
- [ ] T195 Exclude and no-follow strip the complete local-UI lock namespace; route every remaining Guardian/gacha/realm pending consumer through relative handle-bound reads with fail-closed integrity errors; and require completion validation for runtime text and stable save/archive reads.
- [ ] T196 Add deterministic RED regressions for Daren parent/post-image replacement and publication source/destination hard-link races after initial validation, including exact destination restoration/absence and unsupported-platform fail-closed behavior.
- [ ] T197 Retain transaction-scoped Daren post-image authority and implement one reversible physical replacement boundary that retains source/target authority, restores exact prior destination/absence before reporting failure, and rejects unsupported platform fallbacks before mutation.

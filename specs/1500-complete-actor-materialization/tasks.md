...
- [x] T093 Atomically reserve immutable task and proposal IDs across concurrent bridge-pool instances, enforce each worker profile's `MaxConcurrentTasks` limit at runtime, and cover both task and proposal publication races with deterministic concurrency tests.
  - Evidence: RED deterministically launched two workers for one task ID, published two workers into one proposal ID, and published a second task while a `MaxConcurrentTasks = 1` worker was still active. GREEN accepts exactly one task/proposal claimant across independent pools and keeps the second task unpublished until the first slot is released; focused concurrency tests pass 3/3 and the complete bridge lifecycle suite passes 19/19.

...
## Phase 44 - Retained rollback authority and ambient-chain closure

- [x] T239 Record the two confirmed Phase 43 independent-review findings as FR-119, FR-120, SC-035, Decisions 135 and 136, and corresponding plan/data-model changes.
- [x] T240 Add deterministic RED regressions for foreign rewrites between rollback authorization and restore/delete mutation plus overlapping cancelled ambient acquisitions; condition restore/delete on retained physical authority and prune inactive interior registrations.
  - Evidence: all three regressions failed against `810f9c96`: both foreign writes were overwritten/deleted and ambient depth reached four. Retained-handle conditional replacement/deletion and interior predecessor pruning make the same group pass `3/3`; browser/filesystem/proposal/source coverage passes `201/201`, and Daren/reversible-publication coverage passes `127/127`.
- [x] T242 Close the Phase 44 survivor-activation review finding with ascending and descending cancellation-order regressions plus successor-driven inactive-predecessor pruning.
  - Evidence: the surviving pending acquisition first activated with ambient depth `13` after its eleven predecessors were cancelled. Pending and active registrations now track live successors, notify them when a predecessor becomes inactive, and atomically relink around that predecessor. Both cancellation orders retain depth `1` with exactly one active survivor and return to depth zero after disposal plus caller-side observation.
- [x] T243 Replace the successor-registration recursion identified by the follow-up review with an iterative re-registration handshake and guard the method against future recursive pruning.
  - Evidence: the reviewer demonstrated stack exhaustion only with an adversarial synthetic chain of 100,000 inactive registrations. `RegisterSuccessor` now returns registration success without calling back into pruning; the caller iterates until it reaches a retained pending/active predecessor or absence. The browser/filesystem/proposal/source matrix remains green at `203/203`.
  - Follow-up: GitHub issue #1501 tracks the independent review's separate Minor save-archive decompression-bound hardening outside the Phase 44 race closure.
- [x] T244 Synchronize stale full-suite save fixtures with the Phase 43 canonical soul-state requirement without weakening archive validation.

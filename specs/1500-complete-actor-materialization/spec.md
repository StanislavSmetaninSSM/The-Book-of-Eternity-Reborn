...
- **FR-010**: Existing Mortal NPCs MUST continue to mutate through dedicated delta commands; validators MUST forbid using the first-materialization envelope or a null permanent ID plus colliding same-turn `initialId` to bypass existing update contracts. Validation MUST derive one effective identity from canonical permanent/same-turn fields, reject reuse of any validated pre-turn permanent `NPCId` as `initialId` with stable code `npc_initial_id_collides_with_existing_permanent_id`, and apply inventory continuity to that effective identity. A true legacy promotion MAY include the schema-required inventory snapshot only when it is semantically identical to validated pre-turn inventory; any add, removal, or change through `UpdateNPCs.inventory` MUST be rejected and authored through `NPCInventoryAdds`, `NPCInventoryUpdates`, or `NPCInventoryRemovals`.

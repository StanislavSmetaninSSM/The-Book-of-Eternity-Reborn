...
- **FR-032**: After an accepted canonical repair, player-facing output MUST be newer than the latest actual write of each canonical target repaired in that cycle. Repair-request creation time, worker dispatch time, or loop start time MUST NOT substitute for canonical mutation time. If a required repaired target has no trustworthy mutation boundary, freshness MUST fail closed.
...
- **SC-005**: Repair tests show that already valid actor sections remain semantically equivalent, only the exact named repair subtree may change, actor-type-aware memory routing protects unrelated profile/root data, stale-byte proposals and concurrent overwrite/rollback attempts are rejected, omitted/unspecified/non-completed proposal statuses cannot enter apply, audit append races lose no events, every audit producer uses the shared collision-safe ID generator, audit publication failure cannot revoke an accepted apply, characteristic repair cannot exceed hash-pinned setting authority, player output cannot predate repaired canonical bytes, and proposals that alter protected actor data are rejected before apply.

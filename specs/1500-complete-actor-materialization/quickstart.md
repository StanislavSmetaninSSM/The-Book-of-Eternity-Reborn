...
- Generate one unique validation-repair task ID per dispatch while preserving the attempt prefix; atomically reserve task/proposal IDs and acquire the profile's `MaxConcurrentTasks` slot before publishing the task.

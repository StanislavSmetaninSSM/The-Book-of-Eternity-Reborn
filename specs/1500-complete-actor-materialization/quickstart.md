...
- Verify every declared non-delete artifact digest before publication. Reserve task IDs by create-only compare/exchange; stage the proposal plus all declared content and publish the complete bundle with one create-only atomic directory rename only while exact task bytes still identify the current session generation. Keep the durable bundle authoritative if a derived inbox/audit write fails, and keep timeout authoritative when malformed proposal bytes coexist.
- Generate one unique validation-repair task ID per dispatch while preserving the attempt prefix. Acquire the profile's shared `MaxConcurrentTasks` slot before publishing the task, retire reference-counted gates when idle, and on cancellation await the complete killed process tree before releasing the slot.

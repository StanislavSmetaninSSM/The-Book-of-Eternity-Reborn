# Implementation Plan: Daren Heavy-Grate Fail Literary Aftermath

**Branch**: `work/1014-daren-heavy-grate-fail` | **Date**: 2026-06-13 | **Spec**: `specs/1014-daren-heavy-grate-fail/spec.md`

**Input**: Feature specification from `specs/1014-daren-heavy-grate-fail/spec.md`; source GitHub issue [#1014](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/1014); parent [#955](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/955); source scene [#977](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/977); preceding results [#1012](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/1012) and [#1013](https://github.com/StanislavSmetaninSSM/The-Book-of-Eternity-Reborn/issues/1013).

## Summary

Rewrite only Daren QTE `physical_pressure_action` fail result into a substantial Russian dark-fantasy aftermath insert that preserves the dangerous-failure grade: the grate crashes, noise/evidence/pursuit pressure escalates, and Daren salvages the staff under compromised conditions. Add a focused failing guard first so the old terse result is rejected and the final result proves console/browser parity through shared route data.
...
- **GitHub traceability**: PASS. Spec, plan, tasks, checklist, and contract reference #1014 and parent #955.
...
```text
specs/1014-daren-heavy-grate-fail/
├── spec.md
├── plan.md
├── contracts/
...
**Structure Decision**: Keep route result prose in the existing shared C# route data and extend the existing Daren showcase test suite. Do not add frontend, endpoint, state, reward, or documentation contract files outside the #1014 Spec Kit artifacts unless verification exposes a real issue.

## TDD Strategy

1. Add a focused test such as `DarenPhysicalPressureFail_ReadsAsDangerousAftermathWithoutMechanicDrift` in `BookOfEternityClient.Tests/DarenQteShowcaseTests.cs` before changing the fail prose.
2. The guard asserts:
   ...
   - grouped motif coverage for failed heavy-grate escalation, Daren body/breath/control under pressure, staff-case/niche salvage, evidence/noise/witness/pursuit stakes, and next-corridor continuity;
...
3. Run the focused Daren test filter and record RED evidence against the existing terse fail result.
4. Replace only the `physical_pressure` action fail text in `QteSceneService.Daren.cs`.
5. Run focused and affected verification to GREEN.

## Baseline Evidence Before Implementation

- Branch `work/1014-daren-heavy-grate-fail` started from `origin/main` at `3232f7b`; source issue #1014 and parent #955 are the tracked tasks.
- Spec Kit prerequisite check before implementation: `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .specify/scripts/powershell/check-prerequisites.ps1 -Json -RequireTasks -IncludeTasks` resolved `FEATURE_DIR=E:\Games\worktrees\boe-1014-daren-heavy-grate-fail\specs\1014-daren-heavy-grate-fail` with `contracts/` and `tasks.md`.
- `specify version` reported CLI 0.9.3; `specify integration list` reported Codex CLI installed/default.
- Focused Daren baseline command: `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj -p:IsTestProject=true --filter "FullyQualifiedName~DarenQteShowcaseTests" --logger "console;verbosity=minimal"` passed: 53 passed / 0 failed / 0 skipped / 53 total.
- Affected Daren/QTE/docs/browser baseline command: `dotnet test BookOfEternityClient.Tests/BookOfEternityClient.Tests.csproj -p:IsTestProject=true --filter "DarenQteShowcaseTests|QteSceneServiceTests|ValidationServiceQteTests|PromptDocumentationCoverageTests|ExampleDocumentationValidationTests|BrowserApiContractTests|BrowserFrontendWorkspaceTests" --logger "console;verbosity=minimal"` passed: 322 passed / 0 failed / 0 skipped / 322 total.

## Review Requirements

Independent review must check:

- `physical_pressure_action` fail literary quality against issue #1014 and parent #955;
- Daren remains the active protagonist in a dangerous physical-action aftermath;
- the prose clearly communicates failure through noise, evidence, witness, pursuit, alarm, pain, or compromised movement without naming score, QTE, or implementation mechanics;
- the result naturally bridges from the fallen grate/staff salvage toward the next alarm-pulse corridor;
...
- Spec Kit artifacts do not treat #1012/#1013 or parent #955 as current closure scope.

...
public sealed partial class GameEngineTurnLifecycleTests : IDisposable
{
    ...
    public async Task CollectAcceptedTurnRawStateIssuesAsync_DirectNpcCoreMutation_IsRejectedBeforeNormalization()
    {
        ...
        var preTurnRoot = new JsonObject
        {
            ["NPCsInScene"] = new JsonArray
            {
                new JsonObject
                {
                    ["NPCId"] = "npc_archive_mentor",
                    ["NPCName"] = "Смотритель архива",
                    ["worldview"] = "Знание требует осторожности."
                }
            }
        };
    ...
    [Fact]
    public async Task WaitForContractRepairAsync_WhenSessionIsReplaced_AbortsBeforeLegacyRepairOrRollback()
    {
        const string sessionId = "session-worker-repair-replaced";
        const string requestId = "request-worker-repair-replaced";
        const int turnNumber = 5;
        const string trackedPath = "game_state/world/weather.json";
        const string replacementWeather = "{\"replacementSession\":true}";
        ...
        var canonicalGenerationPath = _fs.SessionGenerationPath.Replace("'", "''", StringComparison.Ordinal);
        var canonicalWeatherPath = _fs.ResolvePath(trackedPath).Replace("'", "''", StringComparison.Ordinal);
        var replacementGeneration = Guid.NewGuid().ToString("N");
        var scriptPath = Path.Combine(_rootPath, "fake-validation-repair-session-replaced.ps1");
        await File.WriteAllTextAsync(scriptPath, $$"""
            $task = Get-Content -Raw -Path $env:BOE_WORKER_TASK_PATH | ConvertFrom-Json
            Set-Content -Path '{{canonicalGenerationPath}}' -Value '{"schemaVersion":1,"generationId":"{{replacementGeneration}}"}' -Encoding UTF8 -NoNewline
            Set-Content -Path '{{canonicalWeatherPath}}' -Value '{{replacementWeather}}' -Encoding UTF8 -NoNewline
            $proposalId = 'worker_proposal_session_replaced'
            $contentRef = 'worker_proposals/' + $proposalId + '/game_state/world/weather.json'
            $contentPath = Join-Path $env:BOE_WORKER_SESSION_PATH $contentRef
            New-Item -ItemType Directory -Path (Split-Path $contentPath) -Force | Out-Null
            Set-Content -Path $contentPath -Value '{"description":"stale worker bytes"}' -Encoding UTF8 -NoNewline
            $sha = [System.Security.Cryptography.SHA256]::Create()
            try { $afterSha256 = ([BitConverter]::ToString($sha.ComputeHash([IO.File]::ReadAllBytes($contentPath)))).Replace('-', '').ToLowerInvariant() }
            finally { $sha.Dispose() }
            $proposal = [ordered]@{
                ...
                workerId = $task.workerId
                status = 'completed'
                summary = 'Stale proposal after replacement session became current.'
                changedFiles = @([ordered]@{
                    path = 'game_state/world/weather.json'
                    changeKind = 'replace'
                ...
                selfCheck = [ordered]@{
                    scopeReviewed = $true
                    validationExpectedToPass = $true
                    notes = @('session replacement regression')
                }
                createdAtUtc = '2026-07-23T01:00:05Z'
            }
            $proposal | ConvertTo-Json -Depth 20 | Set-Content -Path $env:BOE_WORKER_PROPOSAL_PATH -Encoding UTF8
            """, Encoding.UTF8);
        var engine = CreateGameEngine(configureSettings: settings =>
    ...
    [Fact]
    public async Task WaitForContractRepairAsync_StaleCapturedSession_AbortsBeforeDispatchAndRollback()
    {
        const string trackedPath = "game_state/world/weather.json";
        const string originalWeather = "{\"description\":\"old session\"}";
        const string replacementWeather = "{\"description\":\"replacement session\"}";
        await _fs.WriteFileAtomicAsync(trackedPath, originalWeather);
        var engine = CreateGameEngine(new QueuedConsoleInputSource([Key(ConsoleKey.Escape)]));
        var rollbackSnapshot = await InvokePrivateTaskResultAsync(
            engine,
            "CreatePreTurnBackup",
            "stale_repair_generation");
    ...
    [Fact]
    public async Task WaitForGmResponse_TerminalErrorDoesNotHoldSessionLifecycleLease()
    {
        const string sessionId = "session-cancel-lifecycle";
        const string requestId = "request-cancel-lifecycle";
        const int turnNumber = 4;
        const string trackedPath = "game_state/world/weather.json";

        await _fs.WriteFileAtomicAsync(trackedPath, "{\"description\":\"stable\"}");
        await _fs.WriteFileAtomicAsync(
    ...
    [Fact]
    public async Task WaitForGmResponse_SessionRotationDuringTerminalWait_ThrowsWithoutCleaningReplacement()
    {
        const string sessionId = "session-terminal-rotation";
        const string requestId = "request-terminal-rotation";
        const int turnNumber = 8;
        const string trackedPath = "game_state/world/weather.json";

        await _fs.WriteFileAtomicAsync(trackedPath, "{\"description\":\"old\"}");
        await _fs.WriteFileAtomicAsync(
            $"game_state/control/pending_turn_snapshot/{trackedPath}",
            "{\"description\":\"old\"}");
        await WritePendingTurnSnapshotManifestAsync(sessionId, requestId, turnNumber, trackedPath);
        ...
        var terminalWaitStarted = new TaskCompletionSource(
            ...
            finalizationHooks: new GameEngineSessionFinalizationHooks
            {
                AtCheckpointAsync = checkpoint =>
                {
                    if (checkpoint == SessionFinalizationCheckpoint.TerminalWaitStarted)
                        terminalWaitStarted.TrySetResult();
                    return Task.CompletedTask;
                }
            });

        var waitTask = InvokePrivateAsync<bool>(engine, "WaitForGmResponse");
        await terminalWaitStarted.Task.WaitAsync(TimeSpan.FromSeconds(5));
    ...
    public async Task RebindRuntimeAfterSessionReplacement_ActiveReplacementRebindsLoopAndClearsTransientState()
    {
        ...
        await _fs.WriteFileAtomicAsync(
            "game_state/meta/soul_state.json",
            """{ "soulName": "Новая душа", "currentRealm": "Chaos Sea", "currentIncarnation": 2 }""");
        await _fs.WriteFileAtomicAsync(
            "game_state/history/chat_log.json",
            """{ "sessionId": "replacement-session", "messages": [] }""");
        await _fs.WriteFileAtomicAsync(
            "stories/replacement.json",
            """{ "turn": 7, "response": "replacement" }""");

        var engine = CreateGameEngine(new QueuedConsoleInputSource([]));
        var gameLoop = GetPrivateField<GameLoop>(engine, "_gameLoop");
        var explorer = GetPrivateField<ExplorerMode>(engine, "_explorer");
        gameLoop.SetSession("old-session", 42);
        SetPrivateField(engine, "_lastResponse", new GameResponse { Response = "old response" });
        SetPrivateField(engine, "_pendingImagePrompt", "old image prompt");
        SetPrivateField(engine, "_pendingMemoryLegacyAwaitingConsumption", true);
        SetPrivateField(engine, "_mainMenuSessionWarning", "old warning");
        SetPrivateField(engine, "_lastConsoleWidth", 180);
        SetPrivateField(engine, "_lastKnownLevel", 9);
        SetPrivateField(engine, "_inGame", true);
        SetPrivateField(explorer, "_pendingGmAction", "old pending action");
        SetPrivateField(explorer, "_currentCommandInput", "/old");
        SetPrivateField(explorer, "_currentCommandRemainder", "old");
        SetPrivateField(explorer, "_diceRevealed", true);

        await InvokePrivateTaskAsync(engine, "RebindRuntimeAfterSessionReplacementAsync");

        Assert.Equal("replacement-session", gameLoop.SessionId);
        Assert.Equal(7, gameLoop.TurnNumber);
        Assert.True(GetPrivateFieldValue<bool>(engine, "_inGame"));
        Assert.Null(GetPrivateFieldValue<GameResponse?>(engine, "_lastResponse"));
        Assert.Null(GetPrivateFieldValue<string?>(engine, "_pendingImagePrompt"));
        Assert.False(GetPrivateFieldValue<bool>(engine, "_pendingMemoryLegacyAwaitingConsumption"));
        Assert.Null(GetPrivateFieldValue<string?>(engine, "_mainMenuSessionWarning"));
        Assert.Equal(0, GetPrivateFieldValue<int>(engine, "_lastConsoleWidth"));
        Assert.Equal(1, GetPrivateFieldValue<int>(engine, "_lastKnownLevel"));
        Assert.Null(GetPrivateFieldValue<string?>(explorer, "_pendingGmAction"));
        Assert.Equal(string.Empty, GetPrivateFieldValue<string>(explorer, "_currentCommandInput"));
        Assert.Equal(string.Empty, GetPrivateFieldValue<string>(explorer, "_currentCommandRemainder"));
        Assert.False(GetPrivateFieldValue<bool>(explorer, "_diceRevealed"));
    }

    [Fact]
    public async Task RebindRuntimeAfterSessionReplacement_EmptyReplacementReturnsToMenu()
    {
        await _fs.ClearGameStateAsync();
        var engine = CreateGameEngine(new QueuedConsoleInputSource([]));
        var gameLoop = GetPrivateField<GameLoop>(engine, "_gameLoop");
    ...
    [Fact]
    public async Task TryPromoteValidationRepairArtifactStallToTerminalErrorAsync_StaleSession_DoesNotReplaceReadySignal()
    {
        const string sessionId = "session-stale-stall";
        const string requestId = "request-stale-stall";
        const int turnNumber = 4;
        const string trackedPath = "game_state/meta/soul_state.json";
        await WriteJsonAsync(trackedPath, new { currentRealm = "Mortal World" });
        await WriteJsonAsync($"game_state/control/pending_turn_snapshot/{trackedPath}", new { currentRealm = "Mortal World" });
        await WritePendingTurnSnapshotManifestAsync(sessionId, requestId, turnNumber, trackedPath);
        await WriteJsonAsync("game_state/control/validation_repair_request.json", new
        {
            sessionId,
        ...
        var engine = CreateGameEngine(new QueuedConsoleInputSource([]));
        var pendingResolution = await InvokePrivateTaskResultAsync(
            engine,
            "ResolveActivePendingTurnSnapshotContextAsync");
        ...
        await Assert.ThrowsAsync<GmWorkerSessionReplacedException>(() =>
            InvokePrivateAsync<bool>(
                engine,
                "TryPromoteValidationRepairArtifactStallToTerminalErrorAsync",
                context,
                staleGeneration));

        Assert.Equal(replacementSignal, await _fs.ReadFileAsync("ready/turn_error.json"));
    }
    ...
    public async Task WaitForContractRepairAsync_AcceptedRepairReady_WritesAcceptedTrajectoryRecord()
    {
        ...
        var issue = new ValidationIssue(
            ...
            IssueSeverity.Error,
            "Current location creation is missing its description.",
            code: "mortal_location_materialization_governed_field_missing",
            actor: "mortal_location:new:locref_repair_ledger",
        ...
        issue.MortalLocationRepairContext = new MortalLocationRepairContext(
            "currentLocationData",
            "mortal_location",
            "locref_repair_ledger",
            "mlocmat_repair_ledger",
            new[] { "description" });
        var repairSessionGeneration = await GetOrCreateSessionGenerationAsync();
    ...
    [Fact]
    public async Task AppendClearedValidationRepairTrajectoryAsync_StaleSession_AbortsRepairCycle()
    {
        var engine = CreateGameEngine(new QueuedConsoleInputSource([]));
        var staleGeneration = await GetOrCreateSessionGenerationAsync();
        await SessionReplacementTestHarness.RotateGenerationAsync(_fs);
        ...
        var exception = await Assert.ThrowsAsync<GmWorkerSessionReplacedException>(() =>
            InvokePrivateTaskAsync(
                engine,
                "AppendClearedValidationRepairTrajectoryAsync",
                "state validation",
        ...
        Assert.False(_fs.FileExists("game_state/control/gm_trajectory_ledger.jsonl"));
    }

    [Fact]
    public async Task RemoveGuardianQuestProgressUpdatesCommandSurfaceAsync_StaleSession_DoesNotRewriteReplacementState()
    {
        var engine = CreateGameEngine(new QueuedConsoleInputSource([]));
        var staleGeneration = await GetOrCreateSessionGenerationAsync();
        await SessionReplacementTestHarness.RotateGenerationAsync(_fs);
        ...
        Assert.Equal(replacement, await _fs.ReadFileAsync("game_state/meta/guardians.json"));
    }
    ...
    [Fact]
    public async Task WriteValidationRepairRequestAsync_FactionMaterializationRepair_MortalBoundedBundle_UsesExactTargetsAndPreservationRules()
    {
        ...
        var issues = new List<ValidationIssue>
        {
            new(
                resourcePath,
                IssueSeverity.Error,
                "Faction materialization does not govern the resources sidecar.",
            ...
            new(
                ...
                code: "faction_materialization_section_content_mismatch",
                actor: coordinate,
                section: "FactionMaterialization",
                expected: "receipt-bound strategic goods",
                actual: "unbound strategic goods"),
            new(
                npcLinkPath,
                IssueSeverity.Error,
        ...
        Assert.Equal("critical", packet.GetProperty("priority").GetString());
        Assert.Contains(
            coordinate,
            packet.GetProperty("title").GetString() ?? string.Empty,
            StringComparison.Ordinal);
        Assert.Equal(
            new[] { coordinate },
        ...
        Assert.Equal(
            ...
            packet.GetProperty("targetFiles").EnumerateArray().Select(item => item.GetString()));
        Assert.Equal(
            new[] { "Templates/MORTAL_FACTION_UPDATE_TEMPLATE.md" },
        ...
        var targetFiles = packet.GetProperty("targetFiles")
            ...
            .Select(item => item.GetString() ?? string.Empty)
            .ToArray();
        Assert.DoesNotContain(targetFiles, path => path.StartsWith("game_state/meta/", StringComparison.Ordinal));
        Assert.DoesNotContain(targetFiles, path => path.StartsWith("game_state/player/", StringComparison.Ordinal));
        Assert.DoesNotContain(targetFiles, path => path.Contains("faction_structure.json", StringComparison.Ordinal));
        ...
        var correctionKeys = packet.GetProperty("exactFieldCorrections")
            .EnumerateArray()
            .Select(correction =>
                $"{correction.GetProperty("path").GetString()}|{correction.GetProperty("code").GetString()}")
            .ToArray();
        Assert.Equal(
            new[]
            {
        ...
        var expectedShape = string.Join(
            "\n",
            packet.GetProperty("expectedShape").EnumerateArray().Select(item => item.GetString()));
        Assert.Contains("materialization v1", expectedShape, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("receipt", expectedShape, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("populated", expectedShape, StringComparison.Ordinal);
        Assert.Contains("empty", expectedShape, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("actor", expectedShape, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("location", expectedShape, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sidecar", expectedShape, StringComparison.OrdinalIgnoreCase);

        var safeRules = string.Join(
            "\n",
            packet.GetProperty("safeCorrectionRules").EnumerateArray().Select(item => item.GetString()));
        Assert.Contains(coordinate, safeRules, StringComparison.Ordinal);
        Assert.Contains("listed", safeRules, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("target", safeRules, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("preserve", safeRules, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("valid", safeRules, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("receipt", safeRules, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("histor", safeRules, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("chronicle", safeRules, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("unrelated faction", safeRules, StringComparison.OrdinalIgnoreCase);

        var steps = string.Join(
            "\n",
            packet.GetProperty("steps").EnumerateArray().Select(item => item.GetString()));
        Assert.Contains("one by one", steps, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("raw faction materialization", steps, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("before normalization", steps, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("Complete-BoeValidationRepair", steps, StringComparison.Ordinal);

        var doNotDo = string.Join(
            "\n",
            packet.GetProperty("doNotDo").EnumerateArray().Select(item => item.GetString()));
        Assert.Contains("another faction", doNotDo, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("materializationId", doNotDo, StringComparison.Ordinal);
        Assert.Contains("validated history", doNotDo, StringComparison.OrdinalIgnoreCase);
        Assert.True(
            doNotDo.Contains("whole-file", StringComparison.OrdinalIgnoreCase) ||
            doNotDo.Contains("whole file", StringComparison.OrdinalIgnoreCase));
        Assert.Contains("name", doNotDo, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("tag", doNotDo, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("description", doNotDo, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("genre", doNotDo, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public async Task WriteValidationRepairRequestAsync_FactionMaterializationRepair_ShiningExactCrossLinks_UsesOnlyNamedAuthorityRoots()
    {
        const string coordinate = "shining_faction:faction_aurora_council";
        const string residentPath =
            "game_state/meta/guardian_abode_residents.json.residents[0].factionId";
        const string profilePath =
            "game_state/meta/afterlife_entity_profiles.json.profiles[0].factionIds[0]";
        const string sarefPath =
            "game_state/meta/main_story_saref_state.json.alliedFactionId";
        var engine = CreateGameEngine();
        var issues = new List<ValidationIssue>
        {
            new(
                residentPath,
                IssueSeverity.Error,
                "Shining faction resident link is missing.",
                code: "faction_materialization_resident_link_missing",
                actor: coordinate,
                section: "FactionMaterialization",
                expected: coordinate,
                actual: "missing"),
            new(
                profilePath,
                IssueSeverity.Error,
                "Shining faction afterlife profile link is missing.",
                code: "faction_materialization_profile_link_missing",
                actor: coordinate,
                section: "FactionMaterialization",
                expected: coordinate,
                actual: "missing"),
            new(
                sarefPath,
                IssueSeverity.Error,
                "Shining faction Saref story link is missing.",
                code: "faction_materialization_story_link_missing",
                actor: coordinate,
                section: "FactionMaterialization",
                expected: coordinate,
                actual: "missing")
        ...
        Assert.Equal(
            ...
            packet.GetProperty("templateRefs")
                .EnumerateArray()
                .Select(item => item.GetString())
        ...
        var targetFiles = packet.GetProperty("targetFiles")
            ...
            .ToArray();
        Assert.DoesNotContain("game_state/meta/guardians.json", targetFiles);
        Assert.DoesNotContain(targetFiles, path => path.Contains("soul", StringComparison.OrdinalIgnoreCase));
        Assert.DoesNotContain(targetFiles, path => path.StartsWith("game_state/factions/", StringComparison.Ordinal));
        Assert.DoesNotContain(
            targetFiles,
            path => path.Contains("afterlife_spiritual_conflict_state.json", StringComparison.Ordinal));
        Assert.DoesNotContain(targetFiles, path => path.StartsWith("game_state/control/", StringComparison.Ordinal));

        Assert.Equal(
            new[] { profilePath, residentPath, sarefPath },
            packet.GetProperty("missingFields").EnumerateArray().Select(item => item.GetString()));
        Assert.Equal(
            new[] { profilePath, residentPath, sarefPath },
    ...
    public async Task WriteValidationRepairRequestAsync_FactionMaterializationRepair_ExactGrouping_PreservesDistinctCoordinates()
    {
        ...
        var issues = new List<ValidationIssue>
        {
            ...
            new(
                shiningPath,
                IssueSeverity.Error,
                "Shining faction materialization receipt is missing.",
                code: "faction_materialization_receipt_missing",
        ...
        var mortalPacket = Assert.Single(
            packets,
            packet => packet.GetProperty("canonicalActorNames")
                .EnumerateArray()
                .Any(item => item.GetString() == mortalCoordinate));
        ...
        Assert.Equal(
            new[] { mortalCoordinate },
            mortalPacket.GetProperty("canonicalActorNames").EnumerateArray().Select(item => item.GetString()));
        Assert.Equal(
        ...
        Assert.Equal(
            ...
            mortalPacket.GetProperty("targetFiles").EnumerateArray().Select(item => item.GetString()));
        Assert.Equal(
        ...
        Assert.Equal(
            new[] { mortalPath },
            mortalPacket.GetProperty("exactFieldCorrections")
                .EnumerateArray()
                .Select(correction => correction.GetProperty("path").GetString()));
        Assert.Equal(
            new[] { shiningPath },
        ...
        var mortalExpectedShape = mortalPacket.GetProperty("expectedShape")
            .EnumerateArray()
            .Select(item => item.GetString())
            .ToArray();
    ...
    [Fact]
    public async Task WriteValidationRepairRequestAsync_FactionMaterializationRepair_MalformedCoordinates_DoNotBroadenValidPacket()
    {
        const string validCoordinate = "mortal_faction:faction_valid";
        const string validUnrelatedPath =
            "game_state/player/player.json.factionMaterialization";
        var engine = CreateGameEngine();
        var issues = new List<ValidationIssue>
        {
            new(
                validUnrelatedPath,
                IssueSeverity.Error,
                "A valid faction coordinate names an unrelated player root.",
                code: "faction_materialization_section_missing",
                actor: validCoordinate,
                section: "FactionMaterialization",
                expected: "bounded faction state",
                actual: "missing"),
            new(
                "game_state/factions/faction_resources.json.entries[0]",
                IssueSeverity.Error,
                "Faction materialization actor is missing.",
                code: "faction_materialization_section_missing",
                section: "FactionMaterialization",
                expected: "exact machine coordinate",
                actual: "missing actor"),
            new(
                "game_state/factions/faction_structure.json.entries[0]",
                IssueSeverity.Error,
                "Faction materialization actor has leading whitespace.",
                code: "faction_materialization_section_missing",
                actor: " mortal_faction:faction_padded",
                section: "FactionMaterialization",
                expected: "exact machine coordinate",
                actual: "padded actor"),
            new(
                "game_state/factions/faction_projects.json.entries[0]",
                IssueSeverity.Error,
                "Faction materialization actor has trailing whitespace.",
                code: "faction_materialization_section_missing",
                actor: "mortal_faction:faction_padded ",
                section: "FactionMaterialization",
                expected: "exact machine coordinate",
            ...
            new(
                ...
                IssueSeverity.Error,
                "Faction materialization actor has an empty suffix.",
                code: "faction_materialization_section_missing",
                actor: "mortal_faction:",
                section: "FactionMaterialization",
                expected: "exact machine coordinate",
                actual: "empty suffix"),
            new(
            ...
            new(
                ...
                code: "faction_materialization_section_missing",
                actor: "Mortal_faction:faction_wrong_case",
                section: "FactionMaterialization",
                expected: "exact machine coordinate",
                actual: "wrong case"),
            new(
                "game_state/meta/shining_abode_state.json.factions[0]",
                IssueSeverity.Error,
                "Faction materialization actor uses an unsupported domain.",
                code: "faction_materialization_section_missing",
                actor: "faction:faction_wrong_domain",
                section: "FactionMaterialization",
                expected: "exact machine coordinate",
                actual: "wrong domain")
        };
    ...
    [InlineData(
        "faction_materialization_section_missing",
        "mortal_faction:faction_closed_prefix",
        "game_state/factions/faction_projects.json.entries[0].materialization",
        "game_state/factions/faction_core.json",
    ...
    [InlineData(
        ...
        "game_state/factions/faction_core.json.factions[0].materialization",
        "game_state/factions/faction_core.json",
        null,
        "already_materialized")]
    public async Task WriteValidationRepairRequestAsync_FactionMaterializationRepair_ClosedIssueFamilies_AreRouted(
        string code,
        string coordinate,
        string issuePath,
        ...
        string classification)
    {
        ...
        var issues = new List<ValidationIssue>
        {
            new(
                issuePath,
                IssueSeverity.Error,
                "Faction materialization requires a bounded repair.",
                code: code,
                actor: coordinate,
                section: "FactionMaterialization",
        ...
        var expectedTargets = new List<string> { primaryTarget };
        if (secondaryTarget != null)
            expectedTargets.Add(secondaryTarget);
        expectedTargets.Sort(StringComparer.Ordinal);
        Assert.Equal(
            expectedTargets,
        ...
        Assert.Equal(issuePath, correction.GetProperty("path").GetString());
        Assert.Equal(code, correction.GetProperty("code").GetString());
        Assert.Contains(
            classification,
            packet.GetProperty("expectedShape").EnumerateArray().Select(item => item.GetString()));
    }
    ...
    [Fact]
    public async Task WriteValidationRepairRequestAsync_LegacyLocationTransitionCodesDoNotEmitLegacyPacket()
    {
        var engine = CreateGameEngine();
        var issues = new List<ValidationIssue>
        {
        ...
        Assert.DoesNotContain(packets, packet =>
            packet.GetProperty("kind").GetString() is
                "mortal_location_transition_repair" or
                "mortal_world_map_adjacency_repair");
    }

    [Fact]
    public async Task WriteValidationRepairRequestAsync_LegacyLocationShapeCodesDoNotEmitLegacyPacket()
    {
        var engine = CreateGameEngine();
        var issues = new List<ValidationIssue>
        {
    ...
    [Fact]
    public async Task WriteValidationRepairRequestAsync_LegacyAdjacencyCodesDoNotEmitLegacyPacket()
    {
        var engine = CreateGameEngine();
        var issues = new List<ValidationIssue>
        {
    ...
    [Fact]
    public async Task WriteValidationRepairRequestAsync_LegacyOutdoorBiomeCodeDoesNotEmitLegacyPacket()
    {
        var engine = CreateGameEngine();
        var issues = new List<ValidationIssue>
        {
    ...
    public async Task WriteValidationRepairRequestAsync_MortalBootstrapMaterializationErrors_AddsHarnessPacket()
    {
        ...
        Assert.Contains(expectedShape, item => item.Contains("structuredGmAuthority.playerSkills", StringComparison.OrdinalIgnoreCase));
        ...
        Assert.Contains(steps, step => step.Contains("structuredGmAuthority.actorCapabilities", StringComparison.OrdinalIgnoreCase) && step.Contains("teacherProfile", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(steps, step => step.Contains("structuredGmAuthority.playerSkills", StringComparison.OrdinalIgnoreCase));
    ...
    public async Task WriteValidationRepairRequestAsync_MortalBootstrapTradeAnchorMissing_AddsMerchantHarnessPacket()
    {
        ...
        var steps = packet.GetProperty("steps").EnumerateArray().Select(item => item.GetString() ?? string.Empty).ToArray();
        Assert.Contains(steps, step => step.Contains("structuredGmAuthority.actorCapabilities", StringComparison.OrdinalIgnoreCase) && step.Contains("tradeState", StringComparison.OrdinalIgnoreCase));
        ...
        Assert.Contains(doNotDo, item => item.Contains("structuredGmAuthority canTrade", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public async Task CheckGmIncarnationTrigger_BootstrapRepairStall_ReturnsFalseAfterRollback()
    {
        ...
        {
            var request = await WaitForTurnRequestAsync();
            var scaffoldRoot = JsonNode.Parse((await _fs.ReadFileAsync(
                MortalBootstrapLocationScaffold.StatePath))!)!.AsObject();
            var locationRequest = scaffoldRoot["locationMaterializationRequest"]!.AsObject();
            var reservation = locationRequest["startReservation"]!.AsObject();
            var initialId = reservation["initialId"]!.GetValue<string>();
            var start = MortalLocationTestFixture.CreateRawLocation(
                reservation["route"]!.GetValue<string>());
            start["initialId"] = initialId;
            start["coordinates"] = reservation["coordinates"]!.DeepClone();
            start["materialization"]!["initialId"] = initialId;
            start["materialization"]!["materializationId"] =
                "mlocmat_bootstrap_repair_stall_start";
            start["materialization"]!["sourceTurn"] =
                locationRequest["turnNumber"]!.DeepClone();
            start["materialization"]!["sourceAuthority"] =
                locationRequest["sourceAuthority"]!.DeepClone();
            await _fs.WriteFileAtomicAsync(
            ...
            var bootstrapValidator = new ValidationService(
                _fs,
                NullLogger<ValidationService>.Instance);
            var rawLocationIssues = await bootstrapValidator
                .ValidateAcceptedTurnRawMortalLocationMaterializationAsync();
            Assert.True(
                rawLocationIssues.All(issue => issue.Severity != IssueSeverity.Error),
                string.Join(
                    " | ",
                    rawLocationIssues
                        .Where(issue => issue.Severity == IssueSeverity.Error)
                        .Select(issue =>
                            $"{issue.Code}:{issue.FilePath}:{issue.Actor}:" +
        ...
        await gmOutputTask.WaitAsync(TimeSpan.FromSeconds(60));
    ...
    private void ArmCanonicalWriteFailure(string relativePath, int matchingWrite = 1)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(relativePath);
        if (matchingWrite < 1)
            throw new ArgumentOutOfRangeException(nameof(matchingWrite));

        _armedCanonicalWriteFailurePath = relativePath;
        _remainingCanonicalWriteFailureMatches = matchingWrite;
    }
    ...
    private Task BeforeCanonicalMutationAsync(string relativePath)
    {
        if (_armedCanonicalWriteFailurePath == null ||
            !string.Equals(
                relativePath,
                _armedCanonicalWriteFailurePath,
                StringComparison.OrdinalIgnoreCase))
        {
            return Task.CompletedTask;
        }

        _remainingCanonicalWriteFailureMatches--;
        if (_remainingCanonicalWriteFailureMatches > 0)
            return Task.CompletedTask;

        _armedCanonicalWriteFailurePath = null;
        return Task.FromException(
            new IOException($"Injected accepted-turn canonical write failure for '{relativePath}'."));
    }
        ...
        object?[]? args)
    {
        ...
        var compatible = instance
            .GetType()
            .GetMethods(BindingFlags.Instance | BindingFlags.NonPublic)
            .Where(method => string.Equals(method.Name, methodName, StringComparison.Ordinal))
            .Where(method => CanInvokeWith(method, supplied))
            .ToArray();

        var exactArity = compatible
            .Where(method => method.GetParameters().Length == supplied.Length)
            .ToArray();
        return exactArity.Length > 0
            ? Assert.Single(exactArity)
            : Assert.Single(compatible);
    }

    private static bool CanInvokeWith(MethodInfo method, IReadOnlyList<object?> supplied)
    {

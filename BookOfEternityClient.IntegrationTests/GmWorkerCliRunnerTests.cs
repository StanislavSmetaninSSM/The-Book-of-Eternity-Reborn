...
public sealed class GmWorkerCliRunnerTests
{
    ...
    [Fact]
    public async Task RunnerRealMode_PreservesEscapedQuotesInAgentArguments()
    {
        var root = CreateTempRoot();
        try
        {
            var sessionPath = Path.Combine(root, "game_session");
            var taskPath = Path.Combine(root, "task.json");
            var proposalPath = Path.Combine(root, "worker_proposals", "inbox", "task-quoted-config", "proposal.json");
            var capturedArgsPath = Path.Combine(root, "captured-agent-args.txt");
            var fakeAgentPath = Path.Combine(root, "fake-agent-quoted-config.ps1");
            Directory.CreateDirectory(sessionPath);
            await File.WriteAllTextAsync(taskPath, """
                {
                  "schemaVersion": 1,
                  "taskId": "worker_task_runner_quoted_config",
                  "workerId": "analysis_codex",
                  "taskType": "analysis",
                  "responseContract": "worker-proposal-v1"
                }
            ...
            await File.WriteAllTextAsync(fakeAgentPath, $$"""
                [Console]::In.ReadToEnd() | Out-Null
                [System.IO.File]::WriteAllText('{{capturedArgsPath}}', ($args -join [Environment]::NewLine), [System.Text.UTF8Encoding]::new($false))
                $proposal = '{"schemaVersion":1,"proposalId":"worker_proposal_runner_quoted_config","taskId":"worker_task_runner_quoted_config","workerId":"analysis_codex","status":"completed","summary":"Fake agent preserved quoted config.","changedFiles":[],"findings":[],"selfCheck":{"scopeReviewed":true,"validationExpectedToPass":true,"notes":[]},"createdAtUtc":"2026-06-20T00:00:00Z"}'
                [System.IO.File]::WriteAllText($env:BOE_WORKER_PROPOSAL_PATH, $proposal, [System.Text.UTF8Encoding]::new($false))
                """, Encoding.UTF8);
            ...
            var result = await RunRunnerAsync(
                [
                    "-AgentCommand",
                    $"powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File \"{fakeAgentPath}\" -Config model_reasoning_effort=\\\"high\\\"",
                    "-TimeoutSeconds",
                    "10"
                ],
                new Dictionary<string, string>
            ...
            Assert.Equal(0, result.ExitCode);
            Assert.True(File.Exists(capturedArgsPath), result.StandardError);
            var capturedArgs = await File.ReadAllTextAsync(capturedArgsPath, Encoding.UTF8);
            Assert.Contains("model_reasoning_effort=\"high\"", capturedArgs, StringComparison.Ordinal);
        }
        finally
        {
            CleanupTempRoot(root);

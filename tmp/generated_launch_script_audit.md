...
2. Write output/narrative_response.json: `{ "response": "narrative text" }`
3. Write output/interface_updates.json: `{ "dialogueOptions": [...], "image_prompt": "..." }`
4. Write output/debug_logs.json: `{ "gm_thoughts_markdown": "..." }`
...
6. **LAST:** Write exactly one terminal signal:
   - `ready/turn_complete.json` for terminal success
   - OR `ready/turn_error.json` for terminal error

...
- If `game_state/control/pending_training_showcase_requests.json` exists, read `Examples/E_CLI_Training_Showcases.txt` and branch by `requestKind`. For `mortal_teacher_showcase`, fill the requested NPC `trainingShowcase` through `UpdateNPCs`; for `afterlife_teacher_showcase`, fill the requested mentor `mentorTrainingShowcase` through `afterlifeEntityProfileUpdates`. Fresh New Game system Guardian profiles for `guard_system_*` actors are client-created starter mentor profiles; update that existing profile instead of creating a second Guardian or replacing it with a bare stub. Always echo `sourceActorSnapshotHash`. For `mortal_training_skill_evolution`, the client has already spent money/current-level XP; write the complete updated Mortal `activeSkillChanges` or `passiveSkillChanges` plus matching `skillMasteryChanges`, and do not charge again. The GM does not spend player money, experience, Чернильные Перья, or Искры Света directly; afterlife purchases remain client-owned through `/обучение` or expensive self-training through `/духовные_искусства`. Training purchase receipts are historical client-owned audits; do not rewrite old receipt hashes or costs just because a teacher or mentor profile changed later.

...
- If `game_state/control/pending_training_showcase_requests.json` exists, read `Examples/E_CLI_Training_Showcases.txt`. For Mortal World, fill the requested NPC `trainingShowcase` through `UpdateNPCs`; for afterlife, fill the requested mentor `mentorTrainingShowcase` through `afterlifeEntityProfileUpdates`. Always echo `sourceActorSnapshotHash`. The GM does not spend player money, experience, Чернильные Перья, or Искры Света, and does not raise player mastery/art tiers directly; the client performs purchases through `/обучение` or expensive self-training through `/духовные_искусства`.
...
3. If this turn changes `dialogueOptions` and/or `image_prompt`, write output/interface_updates.json: `{ "dialogueOptions": [...], "image_prompt": "...", "timestamp": "ISO_8601" }`; otherwise omit the file
   - Each dialogue option is an object with clean player-facing `text`; if an exact hidden submitted value or control tag is needed, put it in optional `inputValue`, not in `text`.

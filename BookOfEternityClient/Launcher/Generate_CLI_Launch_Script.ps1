...
- Mortal Location Materialization v1: read Templates/MORTAL_LOCATION_TRANSITION_TEMPLATE.md before authoring a Mortal scene or map route. Create complete raw candidates only through current_scene_creation, world_map_creation, and world_map_link_creation; existing movement/update/removal binds exact permanent locationId/linkId, and changed selection additionally requires one exact open directed pre-turn current-to-destination link with no requirements and player-known non-hidden visibility. Use storageUpdates/storagesToRemove and threatsToAdd/threatsToUpdate/threatsToRemove/completeThreatActivities for governed children; section dispositions remain immutable creation evidence. world_map.json owns canonical location/link semantics, current_location.json is a client projection, and location_identity_index.json, permanent IDs, receipts, seals, transitions, knownExits, and adjacencyMap are client-owned. For mortal_location_materialization_repair, the validated pre-turn baseline is already restored and the same request requires full-turn resubmission, not an in-place canonical patch.
...
- Training/trade locality is independent of showcase freshness. Existing Mortal actors must match the exact case-sensitive, Unicode-exact current_location.json.locationId through currentLocationId; same-turn created actors may use the exact accepted initialLocationId during that package. Names, aliases, coordinates, case folding, and Unicode normalization never establish locality. Chaos mentors must match the active Guardian and chaosSeaNavigation.currentAbodeId; Shining mentors and player-visible trade factions must resolve to the valid shining_abode_state.currentHallId. Direct trade actions require the actual currentRealm to match the trade domain, and the client will recheck locality immediately before commit after reading prices, offers, and pending state. A cached showcase never authorizes a remote purchase, and no remote actor/faction should be moved merely to satisfy /обучение or /торговля.

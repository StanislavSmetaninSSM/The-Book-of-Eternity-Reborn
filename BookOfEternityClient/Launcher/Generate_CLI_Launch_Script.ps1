...
- For afterlife entity profiles, use example 26 and write game_state/meta/afterlife_entity_profiles.json only through the documented surfaces: afterlifeEntityProfileUpdates, afterlifeEntityCustomStateChanges, afterlifeFateCardUnlocks, afterlifeActorGoalUpdates, afterlifeActorQuestUpdates, afterlifeActorActivityUpdates, completeAfterlifeActorActivities, afterlifeRelationshipChanges, afterlifeRelationshipLockUpdates, afterlifeBreakthroughQuestUpdates, afterlifeEntityProgressionOverrides, and afterlifeSpecialArtLearningReceipts. Fate Cards live in fateCards[] and unlock only through afterlifeFateCardUnlocks with appliedAtTurn, evidence, and guardian/player/political/combat/training effects; locked cards cannot grant active effects. Actor agency stores goals, personalQuests[], currentActivity, and completedActivities[]; every goal/activity needs gmThoughtsSummary, currentActivity must link to current goals.goalId and an active personal quest, and completed activities close through completeAfterlifeActorActivities rather than direct erasure. Relationship gates live in relationships[] with relationshipLock, breakthroughQuestId, redemptionQuestId, pointOfNoReturn, relationshipGateQuests[], and _clear_; use axes trust/romance/rivalry/oath/fear/reverence/debt, require breakthrough at >=50 and redemption or point-of-no-return proof at <=-50, and never use Mortal NPCRelationshipChanges. Special art upgradeCost must use only inkFeathers/lightSparks with at least one positive value, and learning receipts must not grant progression through initialTier. If a special art is used in conflict, include either specialArtAudit with effectNote or specialArtAudits[] when both sides use named special arts; never write both fields on one exchange. Non-player special arts must match the resolved opposition operation used for actionCostAudit.opposition, not the player's exchange.operationType or a stale incomingAction candidate; incomingAction.finalOperationType is authoritative when present. Terminal/free player operations must not include actionCostAudit.player. If final soul death is attempted, record soulDissipationProof with targetStabilityCoefficient and, for the player, terminalGameOver.

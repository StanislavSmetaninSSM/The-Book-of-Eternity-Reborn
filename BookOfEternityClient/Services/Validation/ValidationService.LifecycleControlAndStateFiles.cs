...
public partial class ValidationService
{
    private async Task ValidateNpcStateFiles(List<ValidationIssue> issues)
    {
        await ValidateNpcFile("game_state/npcs/npc_core.json",
            ...
            {
                "UpdateNPCs", "NPCCoreChanges", "NPCsRenameData", "NPCsInScene",
                "UpdateNpcTradeInventoryReceipts", "trainingPurchaseReceipts"
            }, issues);

        if (await NpcCoreChangesCommandIsPresentAsync())
            issues.AddRange(await ValidateNpcCoreChangesBeforeNormalizationAsync());
    ...
    private async Task ValidateMetaMiscStateFiles(List<ValidationIssue> issues)
    {
        ...
        await ValidateFlexibleStateFile(AfterlifeEntityProfileState.StatePath,
            new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                AfterlifeEntityProfileState.CompleteActivitiesProperty,
                AfterlifeEntityProfileState.RelationshipChangesProperty,
                AfterlifeEntityProfileState.RelationshipLockUpdatesProperty,
                AfterlifeEntityProfileState.BreakthroughQuestUpdatesProperty,
                AfterlifeEntityProfileState.MaskAddsProperty,
                AfterlifeEntityProfileState.MaskUpdatesProperty,
                AfterlifeEntityProfileState.MaskRemovalsProperty,
                AfterlifeEntityProfileState.ActiveMaskChangesProperty,
        ...
        await ValidateFlexibleStateFile(AfterlifeChronicleState.StatePath,
            new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            {
                "schemaVersion", AfterlifeChronicleState.ChroniclesProperty,
                AfterlifeChronicleState.UpdateProperty,
                AfterlifeChronicleState.LastInvalidUpdateProperty,
                AfterlifeChronicleState.LastInvalidUpdateReasonProperty,
                "_lastUpdated"
            }, issues, ValidateAfterlifeChronicleStateFile);
        await ValidateStrictTopLevelObjectFileAsync(AfterlifeChronicleState.StatePath,
            new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            {
                "schemaVersion", AfterlifeChronicleState.ChroniclesProperty,
                AfterlifeChronicleState.LastInvalidUpdateProperty,

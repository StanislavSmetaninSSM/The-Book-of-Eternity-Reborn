...
public sealed partial class QteSceneService
{
    ...
    [
        new("approach_manor", "Подступ к поместью", "Дарен лежит в мокрой траве у стены поместья и считает, как фонари стражи расходятся над калиткой. Если он не выберет тень у старой липы сейчас, патруль увидит подступ и вся ночь начнётся с чужого крика."),
        new("informant_parley", "Шёпот Миры", "Под навесом у задней дороги Дарена ждёт Мира Ночная Нить, информаторка с мокрой лентой на запястье и чужими ключами в памяти. Ей нужен ответ, который докажет, что слух не уйдёт к стражникам: от этого зависит смена у галереи и имя человека, который поведёт погоню."),
        new("gadget_infiltration", "Крюк и леска", "Дарен прижимает плечо к холодному камню башни; над ним темнеет балкон, а двор слушает каждое дрожание лески. Крюк нужно зацепить мягко и подняться до первого оклика, иначе металл выдаст путь наверх."),
        new("stealth_crossing", "Галерея без звука", "Дарен входит в галерею, где портреты смотрят из пыли, а сонный страж дышит за тонкой полосой света. Каждый шаг должен утонуть в тишине: один лишний шум разбудит фонарь и закроет путь к кабинету."),
        ...
        new("staff_theft", "Кража посоха", "Дарен снимает посох с бархатных держателей, и вокруг него едва слышно качаются тонкие кольца с тревожным звоном. Добычу нужно уложить на ремень и удержать баланс, иначе подвески объявят всему дому, что украдено."),
        new("pursuit", "Первый рывок", "За спиной Дарена распахивается зал, и капитан Орвальд Шпиль уже кричит во дворе, где открытое окно сужается в полоску ночи. Ему нужен точный момент для рывка: промедление отдаст погоне темп, лицо в фонарях и имя для утреннего доноса."),
        new("chase_chain", "Цепочка дворов", "Дарен несётся через задний двор, низкую стену, телегу и тёмную аллею, вспоминая маршрут как цепочку ударов сердца. Каждый прыжок и поворот должен сбить преследователей со следа, иначе погоня прочитает всю дорогу к мосту."),
        new("hideout_return", "Убежище под мостом", "Под мостом Дарен вжимается в своё убежище, где вода глушит шаги, а тайник ждёт посох под мокрым камнем. Теперь нужно спрятать добычу и зачистить след: если погоня найдёт этот край, ночь станет опасной даже после кражи.")
    ];
    ...
    private static QteAction BuildDarenAction(string beatId, string? nextBeatId)
    {
        ...
        return beatId switch
        {
            ...
            "gadget_infiltration" => Action(
                ...
                "Крюк срывается с края, и Дарен успевает уйти в тень только после собачьего лая во дворе.",
            ...
            "stealth_crossing" => Action(
                ...
                routing,
                "Дарен переводит вес с доски на доску, и галерея остаётся спящей за его спиной.",
                "Один страж шевелится от скрипа, но Дарен удерживает тишину до того, как тот открывает глаза.",
                "Доска отвечает резким треском, и Дарен видит, как в дальнем крыле поднимается тревожный фонарь.",
            ...
            "lock_pick" => Action(
                ...
                routing,
                "Штифты становятся ровно, и Дарен открывает кабинет так тихо, что пыль на ручке не дрожит.",
                "Замок сдаётся, но отмычка царапает накладку; Дарен уносит этот след вместе с тревогой.",
                "Замок щёлкает слишком громко, и Дарен слышит, как за стеной меняется сонное дыхание стражи.",
            ...
            "rune_memory" => Action(
                ...
                routing,
                "Дарен повторяет узор без дрожи, и руны гаснут одна за другой, оставляя футляр без крика.",
                "Одна руна трескается, но Дарен удерживает порядок знаков, пока дверь остаётся открытой.",
            ...
            "timed_rhythm" => Action(
                ...
                routing,
                "Дарен проходит от тени к тени точно между ударами, и сигнальный кристалл не находит его.",
                "Один красный пульс цепляет сапог, но Дарен успевает вырваться из света до тревоги.",
            ...
            "route_decision" => Action(
                ...
                "Дарен бросается к яркой арке, и свет на мгновение ставит его силуэт перед всем двором.",
            ...
            "staff_theft" => Action(
                ...
                null,
                routing,
                "Посох ложится на ремень без звона, и Дарен чувствует, как добыча становится частью его шага.",
                "Один тонкий звон уходит под потолок, но Дарен ловит баланс раньше, чем подвески заговорят хором.",
                "Посох бьёт по подвескам, и Дарену приходится рвануть прочь под ясный голос украденной добычи.",
            ...
            "pursuit" => Action(
                ...
                "Плащ мелькает в фонарях, но Дарен всё же проскальзывает через двор раньше сомкнутых рук.",
            ...
            "hideout_return" => Action(
                ...
                DarenBranchChoiceConfig("success"),
                routing,
                "Дарен запечатывает посох в тайнике под мостом, и погоня проходит выше, не услышав воды.",

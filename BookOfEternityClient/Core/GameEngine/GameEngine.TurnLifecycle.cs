...
In MortalWorldProfile only, prefer the GM turn helper: build a valid $items array, then call Complete-BoeNpcTradeInventoryRequest -RequestId '<requestId>' -Items $items. The helper finds NPCs by NPCId/npcId/id/initialId, writes explicit npc.tradeInventory with matching tradeCycleId/refreshAfterWorldDate/items, and closes the request through UpdateNpcTradeInventoryReceipts.

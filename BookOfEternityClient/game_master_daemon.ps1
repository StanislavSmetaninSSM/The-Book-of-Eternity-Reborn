...
function New-GmExperienceLesson {
    ...
    elseif ($hasMortalBootstrapPlaceholderIssue) {
        "Open game_state/control/mortal_bootstrap_scaffold.json and replace every player-visible scaffold label from the accepted Mortal bootstrap. Do not keep names like 'Стартовая сцена', 'Силы стартовой сцены', 'Путь из стартовой сцены', or 'Ближайший выход из стартовой сцены'. Rename durable current_location/world_map entries, exits/links, and faction_core/faction_resources entries to concrete in-world names grounded in playerAuthoredStart.characterDescription/worldDescription/startingCircumstances. Name every GM-created NPC through setting-aware GM judgment; the client does not create or name starter NPCs."
    }
    elseif ($hasMortalBootstrapContentAnchorIssue) {
        "Open game_state/control/mortal_bootstrap_scaffold.json. playerAuthoredStart is narrative context only and grants no mechanics. The client initializes structuredGmAuthority arrays empty and leaves experience.json empty; it assigns no default XP threshold, carrying capacity, faction resources, influence/control, or universal powerProfile. When you make a setting-aware mechanical decision, record it in the matching playerProgression, carryingRules, factionMechanics, playerSkills, inventoryItems, actorCapabilities, or resources section and write matching canonical state. Preserve explicit GM declarations and worldEventRequirements. Never derive mechanics through keyword matching or leave an explicit GM decision as prose-only narration."
    }
...
function Get-FirstMortalBootstrapPrompt {
    ...
    return " FIRST MORTAL BOOTSTRAP OUTPUT CHECKLIST: do not open large examples first. Open game_state/control/mortal_bootstrap_scaffold.json and '$script:CompactTurnOutputTemplatePath'. Produce the minimum complete first mortal scene: output/narrative_response.json, output/debug_logs.json with NPC Scope, output/interface_updates.json with clear choices, and game_state/control/progression_report.json when progression counts are required. playerAuthoredStart is narrative context, not mechanical authority. The client baseline contains no inferred skills, items, NPCs, capabilities, money, progression, carrying values, faction resources, influence/control, or universal powerProfile; experience.json is an empty structural object. Record every setting-aware mechanical decision explicitly in the matching structuredGmAuthority.playerProgression, carryingRules, factionMechanics, playerSkills, inventoryItems, actorCapabilities, or resources section and write matching complete canonical GM state; preserve those declarations and the worldEventRequirements opening event. Scaffold labels in the baseline are temporary only: before Complete-BoeTurn, replace player-visible names such as 'Стартовая сцена', 'Силы стартовой сцены', 'Путь из стартовой сцены', or 'Ближайший выход из стартовой сцены' with concrete in-world names for the current location, exits/links, and factions. Every NPC first created on this turn requires the complete materialization envelope. If you add or update player-facing Mortal anchors, use the compact Mortal templates for that exact surface. Finish with Complete-BoeTurn -FilesModified as the LAST action; never return idle with no player-facing output."
}
...
- On the first setting-aware progression decision, record the first complete progression tuple in `mortal_bootstrap_scaffold.json.structuredGmAuthority.playerProgression` and write all applicable `playerLevel`, `level`, `currentExperience`, `experience`, `totalExperience`, `experienceForNextLevel`, and `experienceGained` fields together.

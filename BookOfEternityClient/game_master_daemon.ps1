...
$script:GmDocPathDirective = " GM documentation paths are repo-local and authoritative: TaskGuide='$($script:TaskGuideMainPath)', MainExample='$($script:ExampleMainPath)', MortalItemExample='$($script:MortalItemMaterializationExamplePath)', AfterlifeMatrix='$($script:AfterlifeMatrixPath)', AfterlifeTurns='$($script:AfterlifeTurnsExamplePath)'. Do not search other worktrees for GM docs; use these absolute paths."
...
$script:CompactMortalItemTemplatePath = Join-Path $script:GmContextPackRoot "Templates\MORTAL_ITEM_MATERIALIZATION_TEMPLATE.md"
...
function Get-GmExperiencePreferredSurface {
    ...
    if ($joined.Contains("skill") -or
        $joined.Contains("mastery") -or
        $joined.Contains("mortal_bootstrap_explicit_competency_missing")) {
    ...
    if ($joined.Contains("mortal_bootstrap_placeholder_player_visible_name")) {
        return "MORTAL_LOCATION_TRANSITION_TEMPLATE.md"
    }
...
function New-GmExperienceLesson {
    ...
    $hasMortalBootstrapPlaceholderIssue = $joinedIssues.Contains("mortal_bootstrap_placeholder_player_visible_name")
    $hasMortalBootstrapContentAnchorIssue = $joinedIssues.Contains("mortal_bootstrap_explicit_competency_missing") -or
        $joinedIssues.Contains("mortal_bootstrap_world_event_missing")
...
function Write-GmContextPack {
    ...
    $docSpecs = @(
        ...
        @("Examples\E_CLI_Mortal_Item_Materialization.txt", "mortal_item_materialization_examples"),
...
4. Treat `harnessRepairPackets[].targetFiles` as an exact allowlist. A file absent from targetFiles must not be rewritten.
5. Rewrite only listed output artifacts:
   - `output/narrative_response.json`
   - `output/interface_updates.json`
   - `output/debug_logs.json`
...
`output/debug_logs.json` (only when it is listed in `targetFiles`):
...
- Never infer skills, inventory, teaching, trade, combat capability, or identity from a display name, prose, occupation, or setting genre. Names, archetype prose, item types, and genre keywords never grant mechanics. Materialize only structured facts justified by the current world and scene.
- `capabilities` must agree with the full NPC object. Every required entry under `sections` is either `populated` or `empty_by_design`; `empty_by_design` requires a concise in-world reason and keeps the governed real canonical empty field physically present in its exact empty array/object/null shape.
- First materialization requires 3-5 `personalityTraits`; each trait includes `value` as a JSON integer from 1 through 10.
- A new Mortal `UpdateNPCs` actor requires exactly one non-empty location authority: a known canonical `currentLocationId` with null `initialLocationId`, or a valid same-turn `initialLocationId` with null `currentLocationId`. Never omit both or populate both.
...
'@
    $templates += Write-GmContextPackTemplate -RelativePath "Templates\MORTAL_ITEM_MATERIALIZATION_TEMPLATE.md" -Role "compact_mortal_item_materialization_template" -Content @'
# Compact Mortal Item Materialization Template

Use this before every durable ordinary Mortal item creation, repair, transfer,
split/merge/discard, storage move, or NPC trade. Soul Relics and afterlife
artifacts use their afterlife contracts instead.

## Choose exactly one new-root route

| route | sourceAuthority.kind | exact authority |
| --- | --- | --- |
| player_acquisition | turn_outcome | current accepted turn outcome |
| npc_acquisition | npc_inventory_add | exact NPCInventoryAdds owner/entry |
| new_npc_inventory | new_npc | exact genuinely-new NPC creationRef |
| loot_acquisition | loot_template | derived validated loot template key |
| craft_output | craft_request | pending craft request ID |
| trade_output | npc_trade_receipt | exact trade request/receipt and unique slotId |
| quest_reward | quest_reward | exact canonical quest/reward authority |
| storage_placement | location_storage | exact existing location/storage authority |

## Complete raw new root

- `existedId = null`; omit `itemId`, `id`, `initialId`, and
  `materializationReceipt`.
- Add one turn-unique top-level `creationRef`; repeat it exactly in the
  envelope. For trade output it equals the unique current-schema `slotId`.
- Supply every semantic field: name/description/image_prompt/type/group,
  quality/rarity/price/count/weight/volume/durability/maxDurability,
  bonuses/effects/structuredBonuses/combatEffect/customProperties and both
  mechanical-summary fields, equipment fields, container fields including
  `contentsPath`, consumption, readable/sentient fields, disassembly, bond,
  Fate Card, and quest-link fields.
- Add the exact envelope fields: schemaVersion=1, unique materializationId,
  realm=Mortal, route, sourceTurn, sourceAuthority{kind,authorityId},
  creationRef, state=complete, and `materialization.sections`.
- Declare all twelve sections: presentation, physical, mechanics, equipment,
  container, consumption, readableOrSentient, craftingAndDisassembly,
  bondsAndFateCards, questRole, provenance, ownershipAndPlacement.
- `empty_by_design` needs a non-empty in-world reason and the canonical empty
  field shape. `populated` must have matching structured semantic evidence.

Do not create `itemId`, `materializationReceipt`,
`game_state/inventory/item_identity_index.json`, seals, transitions,
retirement, or lineage. The client seals the item only after raw validation.
Receipt-less canonical state is invalid; the game has not shipped and there is
no runtime promotion/migration path.

## Existing physical item

Resolve exact ordinal `itemId`. A transfer removes one source copy, creates one
destination copy, and preserves envelope, receipt, and unrelated semantics.
Canonical `contentsPath` is null or an ordered permanent-parent-item-ID chain;
same-turn raw parents use `creationRef` and are rewritten by the client. A
GM-authored item must not add `isCarried`, `currentLocationId`, or
`currentLocationName`: they are not placement authority. Exact route
destination, carrier, and `contentsPath` govern placement. A
split keeps the source ID and creates a client-derived child; merge keeps the
selected survivor; full discard retires as destroyed.

## Repair

For `mortal_item_materialization_repair`, write only the exact GM-owned targets
listed in `harnessRepairPackets[].targetFiles`. Never repair a protected ID,
receipt, seal, index, transition, carrier history, or lineage. Protected or
unresolved authority is a client rollback, not permission for whole-file
rewrites. For complete worked JSON, open
...
- If `game_state/control/pending_training_showcase_requests.json` contains requestKind `afterlife_teacher_showcase` for a system or freeform Guardian actor, treat the existing Guardian profile in `game_state/meta/afterlife_entity_profiles.json` as client-created authority. Prefer `Complete-BoeTrainingShowcaseRequest -RequestId '<requestId>' -Offers <offers> -Summary '<player-facing summary>'` after dot-sourcing `game_state/control/gm_turn_helper.bootstrap.ps1`; it fills `mentorTrainingShowcase`, copies the requested `sourceActorSnapshotHash`, validates positive costs/caps, and leaves the fulfilled request for client cleanup after accepted turn/view refresh. Manual fallback is `afterlifeEntityProfileUpdates` with `mentorTrainingShowcase`; afterlife standard-art and `spirit_focus` mentor prices are client-owned and normalized to the current cost policy if missing or mismatched. Do not create a second Guardian, do not replace the profile with a bare stub, and do not charge the player.
- If `game_state/control/pending_training_showcase_requests.json` contains requestKind `mortal_training_skill_evolution`, the client has already charged the paid lesson. Current clients use this pending path for mastery-threshold/effect changes; an ordinary first purchase of a new Mortal skill from a fresh showcase is applied locally at mastery level 1. Resolve the pending request by writing the complete updated Mortal skill surface; do not charge money/XP again and do not leave the level-up/effect change as prose only. For these requests, follow `details.targetKind`: the client may normalize a generic showcase offer such as `skill_mastery` into `active_skill_mastery` or `passive_skill_mastery`; legacy/repaired requests may still carry `active_skill_unlock` or `passive_skill_unlock`. Active targets require `activeSkillChanges` plus matching `skillMasteryChanges`; passive targets require `passiveSkillChanges` only, not invented active skills or active mastery entries.
...
"@
    ...
    $script:MortalItemMaterializationExamplePath = Join-Path $script:GmContextPackRoot "Examples\E_CLI_Mortal_Item_Materialization.txt"

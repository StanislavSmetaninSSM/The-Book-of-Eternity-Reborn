...
$script:ActorMaterializationDirective = " Actor Materialization v1 is mandatory for every newly significant Mortal NPC or non-player afterlife actor. Bind one materializationId and materializedAtTurn to one exact actorType:actorId; do not infer identity, skills, inventory, arts, or authority from a display name, prose, occupation, or setting genre. Accepted afterlife profile actor types are guardian, resident, shining_resident, shining_faction_head, radiant_actor, saref_agent, system_actor, and custom_afterlife_actor; source-backed actors still require exact canonical authority. Record capabilities and every required sections disposition as populated or empty_by_design; every empty_by_design entry needs an in-world reason. A legacy actor that first gains teacher, merchant/trade, combat, Actor Brain, or non-vacant leadership authority is promoted into significant structured play and must receive a complete materialization envelope in that same accepted turn. Afterlife canTeach=true requires a positive standard/special art tier, an explicitly teachable positive special art, or another supported positive lesson such as a mentor showcase offer with sourceCap greater than zero; tier/cap zero and a bare mentor flag are not authority. Positive afterlife trade authority is exact and realm-local: in the Chaos Sea use canTrade=true only for the one active Guardian bound to the exact current abode; in the Shining Abode use it only for a non-player faction head whose leadership is secure or contested, whose faction is operational, and whose trade tier is at least 1. When authoritative trade evidence is unavailable at the validation boundary, fail closed with canTrade=false; never infer trading from names, roles, descriptions, or genre vocabulary. A physical resident record may be removed only when the validated pre-turn snapshot contains its exact no-target departure_only pending transfer and current state contains the exact departed_only receipt plus referenced departure history; otherwise preserve the resident and repair the lifecycle chain. A current materialized profile uses exactly one canonical actorType and actorId and no actorRef alias. The first envelope on an existing profile must validate current actor-owned memory; an untouched legacy profile without a new envelope remains readable. Route Guardian memory to the Guardian thought journal, resident memory to the resident thought journal, and common-profile actor memory to profile gmThoughtsSummary. A bounded common-profile repair may change only the exact actor's gmThoughtsSummary and must preserve unrelated actors, root data, currencies, progression, materialization, and scalar fields. A progression ledger or external chronicle is audit/history and never substitutes for actor-owned thought memory. Populated agency requires meaningful goals, non-empty personalQuests, meaningful currentActivity, or non-empty completedActivities; progressionStrategy or a mask alone is not agency. Ordinary console/browser projections hide gmOnly, secret, non-player-visible, and system_actor profiles unless explicit diagnostics are enabled. For an existing NPC or afterlife actor, preserve the historical envelope and use a dedicated delta rather than rewriting materialization. A bounded worker repair may alter only the exact named section; the apply gate mechanically rejects protected actor data changes outside that scope."

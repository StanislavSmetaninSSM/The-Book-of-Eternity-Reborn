...
- If `game_state/control/pending_training_showcase_requests.json` contains requestKind `mortal_training_skill_evolution`, the client has already charged the paid lesson. Resolve it by writing the complete updated Mortal skill surface; do not charge money/XP again and do not leave the level-up as prose only. For these requests, follow `details.targetKind`: the client may normalize a generic showcase offer such as `skill_mastery` into `active_skill_mastery`, `passive_skill_mastery`, `active_skill_unlock`, or `passive_skill_unlock`. Active targets require `activeSkillChanges` plus matching `skillMasteryChanges`; passive targets require `passiveSkillChanges` only, not invented active skills or active mastery entries.

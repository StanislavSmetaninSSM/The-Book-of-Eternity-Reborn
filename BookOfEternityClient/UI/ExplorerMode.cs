...
public class ExplorerMode
{
    ...
    public ExplorerMode(StateManager stateManager, FileSystemManager fs, LocalizationManager loc,
        ...
        Services.SystemGuardianLibraryService? systemGuardianLibraryService = null,
    ...
    private static List<VisibleRivalSoulThread> ReadVisibleRivalSoulThreads(JsonElement root)
    {
        JsonElement arcs;
            ...
            if (arc.TryGetProperty("publicSignals", out var signals) && signals.ValueKind == JsonValueKind.Array)
            {
                foreach (var signal in signals.EnumerateArray())
                {
                    if (signal.ValueKind != JsonValueKind.Object)
                        continue;
                    var visible = signal.TryGetProperty("visibleToPlayer", out var visibleNode) &&
                                  visibleNode.ValueKind == JsonValueKind.True;
                    if (!visible)
                        continue;
        ...
        else
        {
            foreach (var manifestation in arc.Manifestations)
            {
                var headline = manifestation.Kind.Equals("Сигнал", StringComparison.OrdinalIgnoreCase)
                    ? $"Сигнал: {manifestation.Title}"
                    : $"Этап: {manifestation.Title}";
    ...
    private async Task ShowRivalSoulThreads()
    {
        ...
        var visibleArcs = ReadVisibleRivalSoulThreads(doc.RootElement);

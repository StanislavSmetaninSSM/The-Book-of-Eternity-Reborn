...
Tracked tasks: #565, #567, #569, #570, #571, #572, #573, #574, #576, #577, #585, #586, #587, #588, #589, #590, #591, #592, #593, #594, #619, #620, #621, #622, #682, #684, #685, #687, #688, #689, #691, #701, #702, #703, #704, #705, #723, #727, #728, #729, #742, #759, #771, #880, #881, #929
Parent epic: #559
...
#738 player-copy boundary: Player UI speaks to the player. Default Browser Client screens should describe the book, current chapter, scene, settings, audio, help, and command results in Russian in-world or plain player-readable language. Implementation comments stay in code, docs, and advanced mode; endpoint names, DTO/API wording, raw command coverage, file paths, validation internals, raw JSON, and debug/repair details require explicit `Расширенный режим` or a technical disclosure. Generated HTML smoke artifacts are local/offline evidence under `TestResults/browser-smoke/` and are not committed.
...
The React shell does not autoplay. The `Настройки` route explains that sound starts after a player gesture and asks the player to click `Включить музыку` before tab-local music is played; cue previews use their own explicit `Проверить подсказку` click. Those user gestures select the main-menu or in-game playlist, apply the persisted volume/mute values, and catch playback failures with a short Russian player-facing notice about the current tab's sound permission. Missing audio files are treated as a normal local-install state: the UI stays usable, the settings still persist, and the game continues without music or sound cues.

Issue #689 adds the player-facing browser settings profile at `GET /api/client/settings` and `POST /api/client/settings`. The endpoint reads and writes the shared `GameSettings` authority for language, difficulty, explicit `ShowGmThoughts`, music/sound controls, and browser presentation preferences, then keeps the GM-readable `game_state/core/game_settings.json` difficulty projection current. Browser accessibility values (`BrowserFontScalePercent`, `BrowserReducedMotion`, `BrowserContrastFriendly`) are stored in the same config and applied by React as presentation classes/custom properties; they do not create separate gameplay logic.

Settings changes from React are serialized through a queue before calling `POST /api/client/settings`. Server-side writes reuse the shared browser settings gate and the local write coordinator with a settings-specific owner, so pending GM turns, rollback artifacts, and active browser prompt/session locks block config and GM-projection writes instead of being overwritten by a settings slider.
...
Issue #742 wires the `Начать новую главу` launcher path to the existing C# `/world_setup` browser command and prompt-session form. React never owns new-game rules: it opens the C# form, renders player-safe fields, submits prompt answers through `/api/explorer/prompt-sessions/submit`, or shows a truthful unavailable state when the menu action is disabled or missing a safe command. The ordinary launcher sanitizes command-result blocks/notifications before rendering so slash commands, endpoints, raw JSON labels, file paths, and client-owned state identifiers remain advanced-only rather than player-default copy.

Issue #759 keeps the launcher background art local and reviewable: `BookOfEternityClient.WebFrontend/public/main-menu-bg.webp` is the checked-in dark-fantasy main-menu background, and `BookOfEternityClient.WebFrontend/public/main-menu-bg.source.md` records the Pollinations generation source, prompt/art direction, safety review, and local-runtime boundary. The built-frontend smoke coverage writes `TestResults/browser-smoke/main-menu-background-art.html`, a dependency-light local HTML visual smoke artifact, not an automated screenshot, with desktop and narrow frames showing the main menu background enabled for readability review.

Issue #929 adds the first Browser UI asset catalog at `BookOfEternityClient.WebFrontend/public/browser-ui-assets/catalog.md` and a static local asset batch under `BookOfEternityClient.WebFrontend/public/browser-ui-assets/`. The current batch is deterministic local generated/development artwork because the available image preview tool did not expose retrievable files in this run; the catalog records each asset's surface, aspect ratio, prompt/art direction, provenance, final path, and fallback behavior. Runtime Browser UI loads only committed local files through `src/browserUiAssets.ts`; it does not call Codex, image tools, Pollinations, or remote image services. The built-frontend smoke coverage writes `TestResults/browser-smoke/browser-imagegen-assets.html`, a dependency-light local visual-smoke artifact, not an automated screenshot, with desktop and mobile frames for scene fallback, image-block fallback, and status ambient art.
...
Inside the fallback shell's Advanced / developer panel, the shell includes Russian navigation and a filterable command palette for the major play areas:
...
Default `/afterlife_profiles` is player-facing: it omits full profile raw JSON plus hidden actor motivations, hidden Fate Cards, and unrevealed mask internals. The same command may include the full diagnostic profile JSON only when the browser request sets `advancedEnabled=true` or shared settings have `ShowGmThoughts=true`.

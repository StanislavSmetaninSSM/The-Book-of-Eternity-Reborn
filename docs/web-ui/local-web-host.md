...
The default surface includes the #683 contextual action menu in the `Мир` route. It is built from C# command metadata in `/api/game-screen`, groups actions into Russian game sections (`Персонаж / Душа`, `Мир`, `Квесты`, `Карта`, `Фракции`, `Хранители`, `Посмертие`, `Бой`, `Архив`, `Настройки`), and hides raw slash command IDs from normal player cards. Mutating actions show guided form copy, realm availability, disabled reasons, and warning text; actual local writes remain governed by the C# lifecycle/local-write coordinator and future tracked player-default submission work.

...
| `pending_shining_abode_actions.json` | Ordinary active Shining Abode only, with `preparedIncarnationPackage=null`; file contains exactly one active `request`/`requests[0]`, not a GM-managed queue | Canonical `shining_abode_state.json` mutation plus `coreActionReceipts[]` | Receipt matches request `requestId`, `actionType`, status/timing, target ids, cost fields, and action-specific snapshots; cost receipts identify the request, while resource spends/strength/Radiance changes must also be visible in canonical state deltas | Do not process while package handoff is pending; do not route through mortal factions; do not invent a second simultaneous Shining core request | 14, 17, 18 |
| `pending_shining_faction_foundings.json` | Ordinary active Shining Abode | New/updated Shining faction state plus `factionFoundingReceipts[]` | Matching request, faction identity, founder/role data, status/timing, `quotedCostFeathers=25`, `quotedCostLightSparks=15` | Do not create Mortal World `factionDataChanges`; do not omit the cost audit fields from request or receipt | 9 |
...
| `pending_shining_trade_inventory_requests.json` | Ordinary active Shining Abode; supports `requests[]` but the uniqueness key is `(factionId, tradeCycleId)` | Faction `tradeInventory` plus faction `tradeInventoryReceipts[]` | Inventory matches derived tier/slot count/rarity/service multiplier/trade cycle; receipt has `status=ready`, `itemCount`, `soldOutCount`, timing | Do not invent one-slot inventory when request derives more slots; do not omit item `priceInFeathers` or nested `relicData`; do not duplicate the same faction/cycle request | 2, 17 |
...
| `prepare_incarnation_package` | Persist frozen package and clean gates as canonical helper projects | `selectedCardIds[]`, `selectedCards[]`, `generatedDraftVersion` | Pending request and receipt must carry ordered `selectedCards[]` snapshots from canonical gates cards; do not trigger incarnation in the same turn; package uses `generatedFromDraftVersion`, `preparedAtTurn`, `preparedAtUtc` |

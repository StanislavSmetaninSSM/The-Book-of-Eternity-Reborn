...
| combatConditions | боевые условия | Dedicated `combatConditions[]` layer for visible or private afterlife spiritual-combat buffs/debuffs. Kinds are `mark`, `ward`, `burden`, `opening`, and `vow`; each entry needs target, source, affected operations, duration/uses, `counterplay`, and summary. Visible active combatConditions appear in player-facing conflict/log output; hidden/gm_only combatConditions stay private. They modify only legal axes such as `rollMode`, `conflictPosition`, `controlState`, side strain, `tempoAdvantage`, `counterPayoff`, `actionCostAudit`, or `specialArtAudit.effectNote`; no generic passive stat stacking. |
| mark | метка | A `combatConditions[]` kind that makes a named target easier to pressure, track, bind, or punish through a legal audited axis such as condition-backed rollMode or vulnerability in `specialArtAudit.effectNote`. |
| ward | оберег | A `combatConditions[]` kind that protects a side/actor or makes a listed operation safer/costlier for the opposition through `rollMode`, `actionCostAudit`, `tempoAdvantage`, or a documented special-art effect note. |
| burden | бремя | A `combatConditions[]` kind that makes a target's listed operation harder, costlier, riskier, or more punishable through `actionCostAudit`, disadvantage sources, strain risk, or `counterPayoff`. |
| opening | брешь | A `combatConditions[]` kind that exposes a temporary weakness for a named operation; it should be consumed by a specific follow-up, a condition-backed rollMode source, a counter payoff, or an audited position/strain change. |
| vow | обет | A `combatConditions[]` kind that records a spiritual promise, law, or oath as a bounded combat condition with counterplay; it may protect, constrain, or punish only through the same legal axes as other conditions. |
| counterplay | ответ на условие | Required text/list on every active `combatConditions[]` entry explaining how the target can answer it. The GM must create, consume, expire, or clear combatConditions explicitly; expired/consumed entries do not keep active effects. |
...
| afterlife entity profiles | Профили сущностей посмертия | Read-only/player-visible profile sidecar `game_state/meta/afterlife_entity_profiles.json`; GM updates it through `afterlifeEntityProfileUpdates` when significant afterlife actors are created, revealed, or materially changed. The reserved `player_soul` entry is only a partial mirror for player-owned wallet/progression/ordinary arts; the client refreshes those fields from `soul_state.json` and `shining_abode_state.json`. |
| player_soul profile identity | профиль души игрока | Reserved afterlife entity identity: the player profile uses `actorType=player_soul` and `actorId/actorRef=player_soul`; non-player profiles must not use `actorId=player_soul`. In an afterlife spiritual conflict, the player lead may be written as `actorType=player_soul`, `player`, or `soul` with `actorId=player_soul`; it reads ordinary Spiritual Art and Spirit Focus authority from `soul_state.afterlifeCombatProfile`, not from profile auto-progression, and does not need `actorArtTierSnapshot` / `artAuthoritySource`. |
| supportRole | роль поддержки | Canonical role field for `playerSide.supporters[]` and `oppositionSide.supporters[]` inside afterlife spiritual conflicts. Write this directly (`witness`, `safety_anchor`, `ritual_anchor`, etc.); legacy `role` may be normalized into `supportRole` on start, but it is not the preferred authoring shape. |
| afterlifeEntityProfileUpdates | обновления профилей сущностей посмертия | GM-authored response surface for `profiles[]` entries with `actorType`, `actorId`, `currencies`, `progression`, `standardArts`, `specialArts`, `customStates`, `soulDissipationTier`, and `progressionStrategy`. For `actorType=player_soul`, do not use this surface to grant Ink Feathers, Light Sparks, ordinary `standardArts`, enlightenment/radiance progression, or automatic `progressionStrategy`; those fields are overwritten from client-owned state. |
| afterlifeEntityCustomStateChanges | изменения кастомных состояний сущностей посмертия | GM-authored response surface for targeted `customStates` maintenance on a known profile: `statesToAddOrUpdate[]` adds/replaces full state objects, while `statesToRemove[]` deletes ended states so they do not persist forever. The target profile must exist in `profiles[]` or same-turn `afterlifeEntityProfileUpdates[]`; unknown targets and empty operations are repair-blocking. |
| afterlifeEntityProgressionOverrides | принудительная прокачка сущности посмертия | GM-authored override surface for cases where the living world logic should not follow deterministic `progressionStrategy`; each override needs `cycleKey`, `reason`, `summary`, non-empty explicit deltas, and produces `progressionLedger`. `currencyDeltas` accepts only `inkFeathers` / `lightSparks`; `progressionExperienceDeltas` accepts only `enlightenment` / `radiance`. |
| afterlifeSpecialArtLearningReceipts | квитанции обучения особым духовным искусствам | GM-authored receipt surface for roleplay training that teaches the player an entity's `specialArts[]`; each receipt names the teacher, player, `artId`, `learnedAtTurn`, `trainingConditionSatisfied=true`, `roleplayEvidence`, and summary. Teacher profile, player profile, source `specialArts[].artId`, and `canTeachPlayer=true` are mandatory. A learned special art always starts at tier 0; `initialTier` is optional compatibility data only when it is exactly `0`, not GM-authored progression. |
| specialArts | особые духовные искусства | Named variants of standard spiritual actions. Each entry has owner identity matching the enclosing entity profile, `baseOperation`, `costMultiplierPercent`, `upgradeCost`, `effectSummary`, and, if teachable, `trainingConditions`. `upgradeCost` must be non-empty, use only `inkFeathers`/`lightSparks`, and include at least one positive value. |
| specialArts[].combatEffect | боевой эффект особого искусства | Structured afterlife-only combat contract beside `effectSummary`. Current/new teachable entries use `summary`, `trigger`, `mechanicalAxis`, `allowedPayoff`, `limit`, and `auditRequirement` to state when the art matters, which legal axis it can touch (`rollMode`, `conflictPosition`, `controlState`, `sideStrain`, `tempoAdvantage`, `counterPayoff`, `actionEconomy`, `actionCostAudit`, or `combatConditions`), and what `specialArtAudit.effectNote` / `specialArtAudits[].effectNote` must record. It preserves `baseOperation` and never creates Mortal HP/status effects or unlimited passive bonuses. Combat-useful Predvechnye special-art examples must show the unique combat effect beyond the base operation changing an audited result, not just naming a Guardian art as flavor. |
| soulDissipationProof | доказательство развеивания души | Terminal proof inside `recentConflicts[]` for final soul death after a resolved spiritual conflict. Requires actor/target identity, `dissipationTier`, `targetStabilityCoefficient`, turn, victory proof, and explicit GM motivation. |
| targetStabilityCoefficient | коэффициент устойчивости души | Target resistance against final soul death: `max(enlightenment.tier, radiance.tier)` clamped to `0..4`. Развеивание души is legal only when `soulDissipationTier` is strictly greater. |
| terminalGameOver | окончательная смерть души | Player-only terminal marker in `soul_state.json` after legal soul dissipation. Message must be exactly: `Вы мертвы. Ваша душа окончательно развеяна. Загрузите последнее сохранение и попробуйте снова`. |
| specialArtAudit / specialArtAudits | аудит особого духовного искусства / аудиты особых духовных искусств | Required on a conflict exchange that uses a special art: use either `specialArtAudit` for one used art or `specialArtAudits[]` for one object per art when both player and opposition use named special arts in the same exchange; never write both fields on one exchange. Each audit records `artId`, owner identity, `baseOperation`, `costMultiplierPercent`, and `effectNote` describing how the GM applied the special effect. |
...
| `/afterlife_profiles` | `/профили_загробья` | Shows player-facing Профили сущностей посмертия: actor type, resources, progression, `standardArts`, `specialArts`, visible `fateCards[]`, `customStates`, visible goals/activity summaries, known mask/persona summaries, `soulDissipationTier`, danger warnings, non-player `progressionStrategy`, non-player `lastAutoProgressionCycleKey`, and latest non-player `progressionLedger` entry. The `player_soul` profile displays mirrored player-owned currency/progression/ordinary arts and authoritative learned `specialArts[]`; full raw profile JSON, `gmThoughtsSummary`, hidden Fate Cards, and unrevealed mask internals are diagnostics only, available through `ShowGmThoughts` or browser advanced mode. |
...
- If a side has no current-turn `actionCostAudit.<side>`, its final `activeConflict.actionEconomy.<side>.current` must stay unchanged from the validated pre-turn active conflict; do not edit ОД directly at the root. Terminal/free player operations (`withdraw`, `surrender`, `negotiate`) must not include `actionCostAudit.player`; fake terminal/free audits are invalid.
...
- `oppositionOperation`: the opposition's primary answer, incoming operation, or `none`/`passive`; if `incomingAction.finalOperationType` is present, this must match that final operation, not stale `incomingAction.operationType`; otherwise it must match `incomingAction.operationType`.

...
| player_soul profile identity | профиль души игрока | Reserved afterlife entity identity: the player profile uses `actorType=player_soul` and `actorId/actorRef=player_soul`; non-player profiles must not use `actorId=player_soul`. |

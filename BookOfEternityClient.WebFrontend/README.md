...
Issue #929 adds the first Browser UI generated/development asset workflow. `public/browser-ui-assets/catalog.md` lists each committed asset, target surface, aspect ratio, prompt/art direction, provenance, final path, and fallback behavior. The current batch under `public/browser-ui-assets/` is deterministic local generated/development artwork because the available image preview tool did not expose retrievable files in this run; runtime React imports only local paths from `src/browserUiAssets.ts` and does not call Codex, image tools, Pollinations, or remote services. The built-frontend smoke coverage writes `TestResults/browser-smoke/browser-imagegen-assets.html`, a dependency-light local visual-smoke artifact, not an automated screenshot, for desktop and mobile crop/readability review.

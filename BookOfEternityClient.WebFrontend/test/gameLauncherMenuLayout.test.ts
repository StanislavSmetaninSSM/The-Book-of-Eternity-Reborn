...
assert(launcherSource.includes("mode === primaryAction.mode && !disabled ? ' is-primary' : ''"), 'GameLauncher should not apply primary action styling to disabled launcher actions.');
assert(launcherSource.includes("const actionDescription = launcherActionDescription(menu, mode);"), 'GameLauncher should derive button copy from the main-menu action DTO instead of static placeholders.');
assert(launcherSource.includes("const disabledReason = disabled ? launcherActionDisabledReason(menu, mode) : '';"), 'GameLauncher should derive disabled button reasons from the main-menu action DTO.');
assert(!launcherSource.includes("disabled ? 'пока недоступно' : details.description"), 'GameLauncher should not replace disabled main-menu action reasons with a generic unavailable placeholder.');
assert(launcherSource.includes('<span className="launcher-menu__item-copy">{disabled ? disabledReason : actionDescription}</span>'), 'GameLauncher disabled launcher actions should render player-facing DTO reasons inline.');
assert(launcherSource.includes('<span className="launcher-menu__item-affordance" aria-hidden="true">'), 'GameLauncher enabled launcher actions should render a visible arrow or glyph affordance.');
assert(launcherSource.includes("disabled ? 'Закрыто' : 'Открыть'"), 'GameLauncher action affordance should distinguish disabled and enabled actions without technical copy.');
assert(launcherSource.includes('const sessionWarningText = launcherSessionWarningText(menu.session.validationLabel);'), 'GameLauncher should derive a visible warning state from the session validation label.');
assert(launcherSource.includes('{sessionWarningText && <p className="launcher-session-warning" role="status">{sessionWarningText}</p>}'), 'GameLauncher should render session validation warnings through a launcher warning element.');
assert(launcherSource.includes("function launcherSessionWarningText(validationLabel: string): string"), 'GameLauncher should keep validation warning classification local to launcher presentation.');
assert(launcherSource.includes("if (!playerText || isLauncherValidationOkLabel(playerText))"), 'GameLauncher should avoid showing a false warning for valid session labels.');
assert(launcherSource.includes("function isLauncherValidationOkLabel(playerText: string): boolean"), 'GameLauncher should isolate valid-label detection from warning/error detection.');
assert(launcherSource.includes("!/(невалидн|ошиб|предупрежд|warning|error)/i.test(playerText)"), 'GameLauncher should not suppress validation labels that describe warnings or errors.');
assert(launcherSource.includes("onError={(event) => { event.currentTarget.hidden = true; }}"), 'GameLauncher should hide the decorative image if the local art fails to load so no broken image icon appears.');
...
assert(componentsCss.includes('.launcher-art-bg::before {'), 'Launcher background should include a CSS ambient fallback pattern under the image.');
...
assert(componentsCss.includes('.launcher-menu__item-copy {'), 'components.css should style launcher action copy separately from passive muted text.');
assert(componentsCss.includes('.launcher-menu__item-affordance {'), 'components.css should style launcher action arrow/glyph affordance.');
assert(componentsCss.includes(".launcher-menu__item[data-action-state='disabled'] .launcher-menu__item-affordance"), 'components.css should subdue disabled launcher action affordances.');
assert(componentsCss.includes('.launcher-session-warning {'), 'components.css should define a visible launcher warning/accent treatment for validation states.');
